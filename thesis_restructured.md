...
### The Zandman-Slaner School of Electrical Engineering
...
#### 1.1.1 Information-Theoretic Foundations
...
$$C \leq \max_{p(x, x_R)} \min\left\{I(X, X_R; Y_D), \; I(X; Y_R, Y_D | X_R)\right\} \tag{1}
$$ {#eq:relay-capacity-bound}
*Source: [3, Thm. 1], [4, Ch. 16]*
...
$$C_{\text{DF}} = \max_{p(x, x_R)} \min\left\{I(X; Y_R | X_R), \; I(X, X_R; Y_D)\right\} \tag{2}
$$ {#eq:df-capacity}
*Source: [3, Thm. 1]*
...
$$C_{\text{DF}}^{\text{Gaussian}} = \frac{1}{2}\log_2\left(1 + \gamma\right) \tag{3}
...
**Slot 1 (Source → Relay):**
$$y_R = x + n_1, \quad n_1 \sim \mathcal{N}(0, \sigma^2) \tag{4}
$$ {#eq:awgn-hop1}
*Source: [1, Sec. II]*

**Slot 2 (Relay → Destination):**
$$y_D = x_R + n_2, \quad n_2 \sim \mathcal{N}(0, \sigma^2) \tag{5}
$$ {#eq:awgn-hop2}
*Source: [1, Sec. II]*
...
#### 1.1.3 Cooperative Diversity and Practical Relevance
...
$$G = \sqrt{\frac{P_{\text{target}}}{\mathbb{E}[|y_R|^2]}} \tag{6}
$$ {#eq:af-gain}
*Source: [1, Eq. 1]*

The end-to-end SNR for AF relay is: ([@eq:af-snr])

$$\text{SNR}_{\text{eff}}^{\text{AF}} = \frac{\text{SNR}_1 \cdot \text{SNR}_2}{\text{SNR}_1 + \text{SNR}_2 + 1} \tag{7}
$$ {#eq:af-snr}
*Source: [1, Eq. 2]*
...
$$\hat{b}_R = \text{demod}(y_R), \quad x_R = \text{mod}(\hat{b}_R) \tag{8}
$$ {#eq:df-demod-remod}
*Source: [1, Sec. III]*

The end-to-end BER for DF is: ([@eq:df-ber-hops])

$$P_e^{\text{DF}} = P_{e,1} + (1 - P_{e,1}) \cdot P_{e,2} \tag{9}
$$ {#eq:df-ber-hops}
*Source: [1, Eq. 9], [21, Ch. 14]*
...
#### 1.3.1 Theoretical Basis: Universal Approximation and Denoising
...
$$f^*: \mathbb{R}^{2w+1} \to [-1, 1], \quad f^*(y_{i-w}, \dots, y_{i+w}) = \mathbb{E}[x_i \mid y_{i-w}, \dots, y_{i+w}] \tag{10}
$$ {#eq:optimal-denoiser}
*Source: [23, Ch. 5]*
...
$$f^*(y) = \tanh\left(\frac{y}{\sigma^2}\right) \tag{11}
...
$$\mathbb{E}[(\hat{x} - x)^2] = \text{Bias}^2(\hat{x}) + \text{Var}(\hat{x}) + \sigma^2_{\text{irreducible}} \tag{12}
...
#### 1.3.2 Prior Work in Deep Learning for Physical-Layer Processing
...
#### 1.3.3 Neural Network Relay Processing
...
$$\mathcal{L}(\theta) = \frac{1}{N} \sum_{i=1}^{N} (\hat{x}_i - x_i)^2, \quad \hat{x}_i = f_\theta(y_{i-w:i+w}) \tag{13}
$$ {#eq:mse-loss}
*Source: [23, Ch. 5]*
...
$$\log p_\theta(\mathbf{x}) = \underbrace{\mathbb{E}_{q_\phi}\left[\log \frac{p_\theta(\mathbf{x}, \mathbf{z})}{q_\phi(\mathbf{z} | \mathbf{x})}\right]}_{\text{ELBO}(\theta, \phi; \mathbf{x})} + \underbrace{D_{KL}(q_\phi(\mathbf{z} | \mathbf{x}) \| p_\theta(\mathbf{z} | \mathbf{x}))}_{\geq 0} \tag{14}
$$ {#eq:vae-elbo}
*Source: [11, Eq. 2]*
...
$$\mathcal{L}_{\text{VAE}} = \underbrace{\mathbb{E}_{q_\phi(\mathbf{z}|\mathbf{x})}[\log p_\theta(\mathbf{x}|\mathbf{z})]}_{\text{Reconstruction quality}} - \underbrace{\beta \cdot D_{KL}(q_\phi(\mathbf{z}|\mathbf{x}) \| p(\mathbf{z}))}_{\text{Latent space regularity}} \tag{15}
$$ {#eq:vae-loss}
*Source: [11, Eq. 3]*
...
#### 1.4.2 Conditional Generative Adversarial Networks
...
$$\min_G \max_D \; \mathbb{E}_{\mathbf{x} \sim p_{\text{data}}}[\log D(\mathbf{x})] + \mathbb{E}_{\mathbf{z} \sim p_z}[\log(1 - D(G(\mathbf{z})))] \tag{16}
$$ {#eq:gan-minimax}
*Source: [14, Eq. 1]*
...
$$W(p_{\text{data}}, p_G) = \inf_{\gamma \in \Pi(p_{\text{data}}, p_G)} \mathbb{E}_{(\mathbf{x}, \mathbf{y}) \sim \gamma}[\|\mathbf{x} - \mathbf{y}\|] \tag{17}
...
$$\mathcal{L}_G = -\mathbb{E}[D(G(\mathbf{y}, \mathbf{z}), \mathbf{y})] + \lambda_{\text{L1}} \|\hat{\mathbf{x}} - \mathbf{x}\|_1 \tag{19}
$$ {#eq:wgan-gen-loss}
*Source: [13, Eq. 2]*

$$\mathcal{L}_D = \mathbb{E}[D(G(\mathbf{y}, \mathbf{z}), \mathbf{y})] - \mathbb{E}[D(\mathbf{x}, \mathbf{y})] + \lambda_{\text{GP}} \cdot \text{GP} \tag{20}
$$ {#eq:wgan-disc-loss}
*Source: [13, Eq. 2]*
...
#### 1.4.3 Generative vs. Discriminative Paradigms for Relay Processing
...
#### 1.5.1 Transformers and the Attention Mechanism
...
$$\text{Attention}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \text{softmax}\left(\frac{\mathbf{Q}\mathbf{K}^T}{\sqrt{d_k}}\right)\mathbf{V} \tag{21}
$$ {#eq:attention}
*Source: [15, Eq. 1]*
...
$$\text{MultiHead}(\mathbf{X}) = \text{Concat}(\text{head}_1, \dots, \text{head}_h) W^O, \quad \text{head}_i = \text{Attention}(\mathbf{X} W_i^Q, \mathbf{X} W_i^K, \mathbf{X} W_i^V) \tag{22}
...
$$PE_{(pos, 2k)} = \sin(pos / 10000^{2k/d}), \quad PE_{(pos, 2k+1)} = \cos(pos / 10000^{2k/d}) \tag{23}
...
#### 1.5.2 Structured State Space Models
...
$$\dot{\mathbf{x}}(t) = \mathbf{A}\mathbf{x}(t) + \mathbf{B}u(t), \quad y(t) = \mathbf{C}\mathbf{x}(t) + Du(t) \tag{24}
$$ {#eq:ssm-continuous}
*Source: [17, Eq. 1]*
...
$$\bar{\mathbf{A}} = \exp(\Delta \mathbf{A}), \quad \bar{\mathbf{B}} = (\Delta \mathbf{A})^{-1}(\exp(\Delta \mathbf{A}) - \mathbf{I}) \cdot \Delta \mathbf{B} \approx \Delta \mathbf{B} \tag{25}
$$ {#eq:ssm-zoh}
*Source: [17, Eq. 2]*

yields the discrete recurrence: ([@eq:ssm-discrete])

$$\mathbf{x}_k = \bar{\mathbf{A}} \mathbf{x}_{k-1} + \bar{\mathbf{B}} u_k, \quad y_k = \mathbf{C} \mathbf{x}_k + D u_k \tag{26}
$$ {#eq:ssm-discrete}
*Source: [17, Eq. 3]*
...
#### 1.5.3 Mamba: Selective State Spaces
...
$$\Delta_k = \text{softplus}(W_\Delta u_k + b_\Delta), \quad \mathbf{B}_k = W_B u_k, \quad \mathbf{C}_k = W_C u_k \tag{27}
$$ {#eq:mamba-selective}
*Source: [16, Eq. 4]*
...
#### 1.5.4 Mamba-2: Structured State Space Duality
...
$$y_i = \sum_{j \leq i} \mathbf{C}_i^\top \left(\prod_{k=j+1}^{i} \bar{\mathbf{A}}_k\right) \mathbf{B}_j u_j \tag{28}
$$ {#eq:mamba-ssd-output}
*Source: [18, Eq. 2]*

Defining the **SSM matrix** $M \in \mathbb{R}^{n \times n}$ with entries:

$$M_{ij} = \begin{cases} \mathbf{C}_i^\top \bar{\mathbf{A}}_{j+1:i} \mathbf{B}_j & i \geq j \\ 0 & i < j \end{cases} \tag{29}
$$ {#eq:mamba-ssd-matrix}
*Source: [18, Eq. 3]*
...
#### 1.5.5 State Space Models for Signal Processing
...
$$C = \mathbb{E}\left[\log_2 \det\left(\mathbf{I}_{N_r} + \frac{\text{SNR}}{N_t}\mathbf{H}\mathbf{H}^H\right)\right] \tag{30}
$$ {#eq:mimo-capacity}
*Source: [26, Eq. 1], [27, Eq. 1]*
...
$$\mathbf{y} = \mathbf{H}\mathbf{x} + \mathbf{n} \tag{31}
$$ {#eq:mimo-received}
*Source: [19, Ch. 7.2]*
...
$$\hat{\mathbf{x}}_{\text{ZF}} = (\mathbf{H}^H\mathbf{H})^{-1}\mathbf{H}^H\mathbf{y} = \mathbf{x} + (\mathbf{H}^H\mathbf{H})^{-1}\mathbf{H}^H\mathbf{n} \tag{32}
$$ {#eq:zf-equalizer}
*Source: [19, Ch. 7.2], [29]*
...
$$\text{SNR}_k^{\text{ZF}} = \frac{\text{SNR}}{[(\mathbf{H}^H\mathbf{H})^{-1}]_{kk}} \tag{33}
$$ {#eq:zf-snr}
*Source: [19, Ch. 7.2], [29]*
...
$$\hat{\mathbf{x}}_{\text{MMSE}} = (\mathbf{H}^H\mathbf{H} + \sigma^2\mathbf{I})^{-1}\mathbf{H}^H\mathbf{y} \tag{34}
$$ {#eq:mmse-equalizer}
*Source: [19, Ch. 7.2], [29]*
...
$$\text{SINR}_k^{\text{MMSE}} = \frac{1}{[(\mathbf{H}^H\mathbf{H} + \sigma^2\mathbf{I})^{-1}]_{kk}} - 1 \tag{35}
$$ {#eq:mmse-sinr}
*Source: [19, Ch. 7.2], [29]*
...
#### 1.6.3 MIMO with Relay Processing
...
#### 1.8.1 AWGN Channel — Theoretical Analysis

The AWGN channel adds zero-mean Gaussian noise to the transmitted signal: ([@eq:awgn-channel])

$$y = x + n, \quad n \sim \mathcal{N}(0, \sigma^2), \quad \sigma^2 = \frac{P_s}{\text{SNR}_{\text{linear}}} \tag{36}
$$ {#eq:awgn-channel}
*Source: [21, Ch. 4]*
...
$$P_e^{\text{AWGN}} = Q\!\left(\frac{1}{\sigma}\right) = Q\left(\sqrt{\text{SNR}}\right) = \frac{1}{2}\,\text{erfc}\!\left(\sqrt{\frac{\text{SNR}}{2}}\right) \tag{37}
$$ {#eq:awgn-ber}
*Source: [21, Ch. 4, Eq. 4-3-13]*
...
$$P_e^{\text{DF}} = P_{e,1} + P_{e,2} - 2P_{e,1}P_{e,2} \tag{38}
$$ {#eq:df-ber-awgn}
*Source: [1, Eq. 9]*
...
$$\text{SNR}_{\text{eff}}^{\text{AF}} = \frac{\text{SNR}_1 \cdot \text{SNR}_2}{\text{SNR}_1 + \text{SNR}_2 + 1} = \frac{\gamma^2}{2\gamma + 1} \tag{39}
$$ {#eq:af-snr-eff}
*Source: [1, Eq. 2]*
...
#### 1.8.2 Rayleigh Fading Channel — Theoretical Analysis

The Rayleigh fading channel models non-line-of-sight (NLOS) propagation where the signal undergoes multiplicative fading:

$$y = hx + n, \quad h \sim \mathcal{CN}(0, 1), \quad n \sim \mathcal{CN}(0, \sigma^2) \tag{40}
$$ {#eq:rayleigh-channel}
*Source: [21, Ch. 14]*
...
$$f_{|h|}(r) = 2r \cdot e^{-r^2}, \quad r \geq 0 \tag{41}
$$ {#eq:rayleigh-pdf}
*Source: [21, Ch. 14, Eq. 14-4-1]*
...
$$P_e^{\text{Rayleigh}} = \frac{1}{2}\left(1 - \sqrt{\frac{\bar{\gamma}}{1 + \bar{\gamma}}}\right) \tag{42}
$$ {#eq:rayleigh-ber}
*Source: [21, Eq. 14-4-15]*

where $\bar{\gamma} = \text{SNR}$ is the average SNR. At high SNR ($\bar{\gamma} \gg 1$):

$$P_e^{\text{Rayleigh}} \approx \frac{1}{4\bar{\gamma}} \tag{43}
...
$$P_e^{\text{DF,Rayleigh}} = 2P_e^{\text{Rayleigh}}(1 - P_e^{\text{Rayleigh}}) \tag{44}
$$ {#eq:df-ber-rayleigh}
*Source: [1, Eq. 9]*
...
$$P_e^{\text{Rician}} = \frac{1}{\pi} \int_0^{\pi/2} M_{\gamma}\left(\frac{-1}{\sin^2\theta}\right) d\theta \tag{47}
$$ {#eq:rician-ber-mgf}
*Source: [22, Ch. 8, Eq. 8.98]*

where the MGF of the instantaneous SNR $\gamma$ under Rician fading is:

$$M_{\gamma}(s) = \frac{1+K}{1+K - s\bar{\gamma}} \cdot \exp\left(\frac{Ks\bar{\gamma}}{1+K-s\bar{\gamma}}\right) \tag{48}
$$ {#eq:rician-mgf}
*Source: [22, Ch. 8, Eq. 8.99]*
...
#### 1.8.4 Fading Coefficient Distributions
...
#### 1.8.5 2×2 MIMO Channel — Theoretical Analysis

The 2×2 MIMO spatial multiplexing system transmits two independent BPSK streams simultaneously:

$$\mathbf{y} = \mathbf{H}\mathbf{x} + \mathbf{n}, \quad \mathbf{H} \in \mathbb{C}^{2 \times 2}, \quad H_{ij} \sim \mathcal{CN}(0, 1), \quad \mathbf{n} \sim \mathcal{CN}(\mathbf{0}, \sigma^2\mathbf{I}) \tag{49}
...
$$P_e^{\text{ZF}} \approx \frac{1}{2}\left(1 - \sqrt{\frac{\bar{\gamma}}{1 + \bar{\gamma}}}\right) \tag{50}
$$ {#eq:zf-ber-mimo}
*Source: [19, Ch. 7.2], [28]*
...
$$P_e^{\text{MMSE}} \approx \frac{1}{2}\left(1 - \sqrt{\frac{\gamma_{\text{eff}}}{1 + \gamma_{\text{eff}}}}\right), \quad \gamma_{\text{eff}} \approx \bar{\gamma} \cdot \left(1 + \frac{1}{\bar{\gamma} + 1}\right) \tag{51}
$$ {#eq:mmse-ber-mimo}
*Source: [19, Ch. 7.2], [29]*
...
#### 1.8.6 Channel Model Validation (Simulative)
...
**Zero-Forcing (ZF):**
$$\hat{\mathbf{x}}_{\text{ZF}} = (\mathbf{H}^H\mathbf{H})^{-1}\mathbf{H}^H\mathbf{y} = \mathbf{H}^{-1}\mathbf{y} \tag{52}
...
**MMSE:**
$$\hat{\mathbf{x}}_{\text{MMSE}} = (\mathbf{H}^H\mathbf{H} + \sigma^2\mathbf{I})^{-1}\mathbf{H}^H\mathbf{y} \tag{53}
...
$$\text{Source} \xrightarrow{\text{Hop 1}} \text{Relay} \xrightarrow{\text{Hop 2}} \text{Destination} \tag{54}
$$ {#eq:two-hop-system}
...
$$x_R \leftarrow x_R \cdot \sqrt{\frac{P_{\text{target}}}{P_{\text{current}}}} \tag{56}
...
$$y_R = x + n_1, \quad n_1 \sim \mathcal{N}(0, \sigma^2) \tag{57}
...
$$\mathbf{h} = \text{ReLU}(\mathbf{W}_1 \mathbf{w} + \mathbf{b}_1), \quad \hat{x} = \tanh(\mathbf{W}_2 \mathbf{h} + \mathbf{b}_2) \tag{59}
$$ {#eq:mlp-relay}
*Source: [23, Ch. 6]*
...
#### 3.3.1 Monte Carlo BER Estimation
...
$$\text{CI}_{95\%} = \bar{P}_e \pm t_{0.025, M-1} \cdot \frac{s}{\sqrt{M}} \tag{61}
...
#### 3.3.4 Reproducibility and Weight Management
...
$$x = 1 - 2b, \quad b \in \{0, 1\} \implies x \in \{-1, +1\} \tag{63}
...
#### 3.5.2 QPSK — Gray-Coded Quadrature Phase-Shift Keying

Quadrature Phase-Shift Keying maps pairs of bits $(b_0, b_1)$ to complex symbols:

$$x = \frac{(1 - 2b_0) + j(1 - 2b_1)}{\sqrt{2}} \tag{64}
$$ {#eq:qpsk-mapping}
...
$$P_b^{\text{QPSK}} = Q\!\left(\sqrt{\frac{2E_b}{N_0}}\right) = P_b^{\text{BPSK}} \tag{65}
$$ {#eq:qpsk-ber}
*Source: [21, Ch. 4, Eq. 4-3-31]*

The advantage of QPSK is doubled throughput for the same BER and per-bit energy.

#### 3.5.3 16-QAM — Gray-Coded Quadrature Amplitude Modulation
...
$$I = \frac{L(b_0, b_1)}{\sqrt{10}}, \quad Q = \frac{L(b_2, b_3)}{\sqrt{10}}, \quad x = I + jQ \tag{66}
$$ {#eq:qam16-mapping}
...
$$P_b^{\text{16-QAM}} \approx \frac{3}{8} \operatorname{erfc}\!\left(\sqrt{\frac{2E_b}{5N_0}}\right) \tag{67}
$$ {#eq:qam16-ber}
...
#### 3.5.4 Constellation Diagram Summary
...
#### 3.5.5 16-PSK — Gray-Coded Phase-Shift Keying
...
$$x_k = e^{j \theta_k}, \quad \theta_k = \frac{2\pi k}{16}, \quad k = 0, 1, \ldots, 15 \tag{68}
$$ {#eq:psk16-mapping}
...
$$\hat{k} = \arg\min_{k \in \{0,\ldots,15\}} \left| \angle \hat{x} - \theta_k \right| \tag{69}
$$ {#eq:psk16-decision}
*Source: [21, Ch. 5]*
...
$$P_b^{\text{16-PSK}} \approx \frac{1}{4} \operatorname{erfc}\!\left(\sqrt{\frac{E_b}{N_0}} \sin\!\left(\frac{\pi}{16}\right)\right) \tag{70}
$$ {#eq:psk16-ber}
...
#### 3.5.6 I/Q Splitting for AI Relay Processing of Complex Constellations
...
$$\hat{x}_R = f_\theta(\text{Re}(y_R)) + j \cdot f_\theta(\text{Im}(y_R)) \tag{71}
$$ {#eq:iq-splitting}
...
#### 3.5.7 2D Joint Classification as an Alternative to I/Q Splitting
...
$$\hat{x}_R = s_{\hat{k}}, \quad \hat{k} = \arg\max_{k \in \{1, \dots, M\}} \, f_\theta(y_I, y_Q)_k \tag{72}
...
*Figure 25: 16-QAM on AWGN — AI relays hit a BER floor near 0.22 at medium-high SNR due to $\tanh$ compression of multi-level signals; AF outperforms DF at low SNR (Y*

![Figure 26: 16-QAM relay comparison on Rayleigh fading.](results/modulation/qam16__rayleigh_ci.png)

*Figure 26: 16-QAM on Rayleigh fading — wider BER gap between modulations under fading; all AI relays significantly worse than DF at every SNR point (N*
...
#### 5.1.1 Selective Low-SNR Advantage of Neural Relays (H1: Partially Confirmed)
...
$$f^*(y) = \mathbb{E}[x | y] = \tanh\left(\frac{y}{\sigma^2}\right) \tag{73}
...
#### 5.1.2 Classical Dominance at High SNR (H2: Confirmed)
...
#### 5.1.4 MIMO Equalization Hierarchy (H6: Confirmed)
...
### 5.2 The "Less is More" Principle (H3: Confirmed)
...
#### 5.2.1 Information-Theoretic Perspective
...
$$\text{MSE}(\hat{x}) = \underbrace{(\mathbb{E}[\hat{x}] - f^*(y))^2}_{\text{Bias}^2} + \underbrace{\mathbb{E}[(\hat{x} - \mathbb{E}[\hat{x}])^2]}_{\text{Variance}} + \underbrace{\sigma^2_{\text{irred}}}_{\text{Noise floor}} \tag{74}
...
#### 5.3.1 Context-Length Benchmark: Validating the Crossover Hypothesis
