...
\ifPDFTeX
  ...
  \usepackage{textcomp} % provide euro and other symbolsC:\Users\gzukerma\OneDrive - Qualcomm\Documents\relaynet2C:\Users\gzukerma\OneDrive - Qualcomm\Documents\relaynet2C:\Users\gzukerma\OneDrive - Qualcomm\Documents\relaynet2
...
\urlstyle{same}
\hypersetup{
  pdftitle={1 Deep Learning for Two-Hop Relay Communication: A Comparative Study of Classical and Neural Network-Based Strategies},
...
\title{1 Deep Learning for Two-Hop Relay Communication: A Comparative Study of Classical and Neural Network-Based Strategies}
\author{Gil Zukerma}
\date{2026}
...
\chapter*{Table of Contents}\label{sec:table-of-contents}
...
II & \hyperref[abstract-hebrew]{Abstract (Hebrew)} \\
III & Keywords \\
\end{longtable}
}
...
\tightlist
\item
  \hyperref[introduction-and-literature-review]{Introduction and Literature Review}
  ...
  \end{itemize}
\item
  \hyperref[research-objectives]{Research Objectives}
...
\item
  ...
  \end{itemize}
\item
  \hyperref[discussion-and-conclusions]{Discussion and Conclusions}
...
\chapter*{List of Figures}\label{sec:list-of-figures}
...
26 & 16-QAM on Rayleigh fading --- wider BER gap between modulations under fading; all AI relays \ldots{} & §4.5 Higher-Order Mod. \\
27 & Combined modulation comparison (AWGN) --- all nine relays across BPSK (solid), QPSK (dashed\ldots{} & §4.5 Higher-Order Mod. \\
...
\chapter*{List of Tables}\label{sec:list-of-tables}
...
13 & Model complexity and timing comparison (experiment-specific training settings; Monte \ldots{} & §4.4 \\
14 & BER comparison across modulations at selected SNR points (AWGN channel). All nine rel\ldots{} & §4.5 \\
15 & BER at 16 dB for all relay variants across activation functions and modulations (AWGN\ldots{} & §4.5 \\
16 & 16-QAM BER improvements at 20 dB for sequence models with and without CSI injection a\ldots{} & §4.6 \\
17 & Research positioning: comparison of this thesis against prior work in deep-learning-b\ldots{} & §1.7 \\
18 & Sample BER values comparing blind spatial tracking vs.~explicit Channel State injecti\ldots{} & §4.6 \\
19 & BER at selected SNR points for the top-3 neural relays and classical baselines (16-QA\ldots{} & §4.6 \\
20 & BER at selected SNR points for the top-3 neural relays and classical baselines (16-PS\ldots{} & §4.6 \\
...
24 & BER at 20 dB for all relay variants: 4-class (I/Q split) vs.~16-class (joint 2D) clas\ldots{} & §4.7 \\
25 & Context-length benchmark --- three sequence models at \(n = 255\) on CUDA. & App. B \\
26 & Theoretical SNR required to achieve BER = \(10^{-3}\) and diversity order for each chan\ldots{} & §1.8 \\
27 & MIMO system components, the problem each solves, and their input/output mapping. & §3.1 \\
28 & Normalized 3K-parameter model configurations: parameter count and scaling method for \ldots{} & §3.4 \\
...
}\end{equation} \footnotesize\{\textit{Source: \1}\}\normalsize
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWGN channel --- theoretical BER (solid lines) vs.~Monte Carlo simulation (markers with 95\% CI) for single-hop, two-hop AF, and two-hop DF.}]{results/channel_theoretical_awgn.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rayleigh fading --- theory vs.~simulation for single-hop and two-hop DF. The characteristic 1/\textbackslash text\{SNR\} slope (compared to the steeper exponential AWGN curve) is clearly visible.}]{results/channel_theoretical_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rician fading (K=3) --- theory vs.~simulation for single-hop and two-hop DF.}]{results/channel_theoretical_rician.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Left --- PDF of \textbar h\textbar{} for Rayleigh and Rician (K=1, 3, 10). Right --- CDF (outage probability). Rayleigh has the highest deep-fade probability at any threshold.}]{results/channel_fading_pdf.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={2×2 MIMO Rayleigh --- single-hop BER with ZF, MMSE, and SIC equalization. Theoretical approximations (solid/dashed lines) overlaid with Monte Carlo simulation (markers with 95\% CI). MMSE provides \textasciitilde1--2 dB gain over ZF; SIC provides an additional \textasciitilde0.5--1 dB gain.}]{results/mimo_equalizer_comparison.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Consolidated 2×3 grid of all channel model validations. Top row: (a) AWGN, (b) Rayleigh, (c) Rician K=3 --- theory (blue) vs.~simulation (red). Bottom row: (d) all SISO channels compared, (e) fading PDFs, (f) MIMO equalizer comparison.}]{results/channel_analysis_summary.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Single-hop BPSK BER for all three SISO channel models. AWGN provides the best BER (no fading), Rician K=3 is intermediate, and Rayleigh is the most challenging. The SNR penalty for Rayleigh relative to AWGN exceeds 15 dB at \textbackslash text\{BER\} = 10\^{}\{-3\}, motivating the use of diversity techniques such as MIMO.}]{results/channel_comparison_all.png}}
...
\emph{Rationale:} The bias-variance analysis (Section 1.3.1) predicts that the low-complexity target function \(f^*\) requires minimal model capacity. Excess capacity increases variance without reducing bias.
...
\tightlist
\item
  \textbf{Modulation:} Primary experiments use BPSK. Extension experiments with QPSK and 16-QAM are presented in Section 4.10, and 16-PSK in Section 4.15, to evaluate hypothesis generalisability; however, 64-QAM and higher-order constellations are deferred to future work.
\item
...
\NormalTok{    SRC["**Source**\textbackslash{}ntx bits"]}
\NormalTok{    BPSK["**BPSK**\textbackslash{}nModulator"]}
\NormalTok{    CH1["**Hop 1 Channel**\textbackslash{}nAWGN / Rayleigh / Rician"]}
\NormalTok{    RELAY["**Relay**\textbackslash{}nNeural Net\textbackslash{}ndenoise"]}
\NormalTok{    CH2["**Hop 2 Channel**\textbackslash{}n2x2 MIMO Rayleigh\textbackslash{}ny = H·x\_R + n\textbackslash{}nH = h11 h12 / h21 h22"]}
\NormalTok{    EQ["**Equalizer**\textbackslash{}nZF / MMSE / SIC\textbackslash{}nseparate streams"]}
\NormalTok{    DST["**Destination**\textbackslash{}nrecovered bits"]}

\NormalTok{    SRC {-}{-}\textgreater{} BPSK}
\NormalTok{    BPSK {-}{-}\textgreater{} CH1}
\NormalTok{    CH1 {-}{-}\textgreater{}|"Hop 1"| RELAY}
\NormalTok{    RELAY {-}{-}\textgreater{}|"Hop 2"| CH2}
\NormalTok{    CH2 {-}{-}\textgreater{} EQ}
\NormalTok{    EQ {-}{-}\textgreater{} DST}

\NormalTok{    style SRC   fill:\#fff,color:\#0,stroke:\#0}
\NormalTok{    style BPSK  fill:\#fff,color:\#0,stroke:\#0}
\NormalTok{    style CH1   fill:\#fff,color:\#0,stroke:\#0}
\NormalTok{    style RELAY fill:\#fff,color:\#0,stroke:\#0}
\NormalTok{    style CH2   fill:\#fff,color:\#0,stroke:\#0}
\NormalTok{    style EQ    fill:\#fff,color:\#0,stroke:\#0}
\NormalTok{    style DST   fill:\#fff,color:\#0,stroke:\#0}
\end{Highlighting}
\end{Shaded}
...
\textbf{Hop Model.} Each hop applies a channel function followed by optional equalization: (Equation~\ref{eq:qpsk-mapping})

\[y = h(x, \text{SNR}) + n
\] \{\#eq: (Equation~\ref{eq:psk16-mapping})channel-function\}
...
\NormalTok{flowchart LR}
\NormalTok{    SRC(["**Source**\textbackslash{}ntx bits"])}
\NormalTok{    MOD["**BPSK**\textbackslash{}nModulator"]}
\NormalTok{    CH1["**Hop 1 Channel**\textbackslash{}nAWGN / Rayleigh / Rician"]}
\NormalTok{    REL["**Relay**\textbackslash{}nNeural Net\textbackslash{}ndenoise Hop 1 noise"]}
\NormalTok{    CH2["**Hop 2 Channel**\textbackslash{}n2x2 MIMO · 4 Rayleigh links\textbackslash{}ny = H·x\_R + n\textbackslash{}nH = \&\#91;h₁₁ h₁₂\&\#93; / \&\#91;h₂₁ h₂₂\&\#93;"]}
\NormalTok{    EQ["**Equalizer**\textbackslash{}nZF / MMSE / SIC\textbackslash{}ncancel inter{-}stream interference"]}
\NormalTok{    DST(["**Destination**\textbackslash{}nrecovered bits"])}

\NormalTok{    SRC {-}{-}\textgreater{} MOD}
\NormalTok{    MOD {-}{-}\textgreater{}|"tx symbol x"| CH1}
\NormalTok{    CH1 {-}{-}\textgreater{}|"Hop 1\textbackslash{}nnoisy y\_R"| REL}
\NormalTok{    REL {-}{-}\textgreater{}|"Hop 2\textbackslash{}nclean x\_R"| CH2}
\NormalTok{    CH2 {-}{-}\textgreater{}|"mixed streams\textbackslash{}ny = Hx\_R + n"| EQ}
\NormalTok{    EQ {-}{-}\textgreater{} DST}

\NormalTok{    style SRC  fill:\#4A90D9,color:\#fff,stroke:\#2c5f8a,rx:20}
\NormalTok{    style DST  fill:\#4A90D9,color:\#fff,stroke:\#2c5f8a,rx:20}
\NormalTok{    style MOD  fill:\#7B68EE,color:\#fff,stroke:\#4a3fa0}
\NormalTok{    style CH1  fill:\#E8A838,color:\#fff,stroke:\#b07820}
\NormalTok{    style REL  fill:\#2ECC71,color:\#fff,stroke:\#1a8a4a}
\NormalTok{    style CH2  fill:\#E8A838,color:\#fff,stroke:\#b07820}
\NormalTok{    style EQ   fill:\#9B59B6,color:\#fff,stroke:\#6c3483}
\end{Highlighting}
\end{Shaded}

\textbf{Figure 1 --- End-to-end two-hop relay signal flow.} The relay's neural network solves a \emph{denoising} problem (Hop 1 noise removal); the MIMO equalizer solves an \emph{interference cancellation} problem (Hop 2 stream separation). The two stages are independent and their gains are additive.
...
\begin{itemize}
\item
  \textbf{AF:} Amplifies with gain \(G = \sqrt{P_{\text{target}} / \mathbb{E}[|y_R|^2]}\). AF serves as the lower baseline: it performs no intelligent processing and simply rescales the received signal, preserving both signal and noise. Its theoretical BER is given in Section 3.2.1.
\item
...
BER is estimated through Monte Carlo simulation, which provides an unbiased estimate of the true BER at each SNR point. The simulation parameters are: (Equation~\ref{eq:ber-estimate})
...
Binary Phase-Shift Keying maps a single bit to a real-valued symbol: (Equation~\ref{eq:af-gain})

\[x = 1 - 2b, \quad b \in \{0, 1\} \implies x \in \{-1, +1\}
\] \{\#eq: (Equation~\ref{eq:qam16-mapping})bpsk-mapping\}
...
Figure 8 presents the constellation diagrams for all four modulation schemes used in this thesis. The progression from BPSK (2 points on the real axis) through QPSK (4 points on the unit circle) to 16-QAM (16 points on a rectangular grid) and 16-PSK (16 points on the unit circle) illustrates the fundamental trade-off between spectral efficiency and noise robustness: (Equation~\ref{eq:psk16-ber}) higher-order constellations pack more bits per symbol but reduce the minimum Euclidean distance between points, increasing the BER at a given SNR.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Constellation diagrams. (a) BPSK: 2 real-valued symbols at \textbackslash pm 1. (b) QPSK: 4 Gray-coded symbols on the unit circle. (c) 16-QAM: 16 Gray-coded symbols on a 4 \textbackslash times 4 rectangular grid with decision boundaries (dotted lines). (d) 16-PSK: 16 Gray-coded symbols uniformly spaced on the unit circle. All constellations are normalised to unit average power.}]{results/modulation/constellation_diagrams.png}}
...
16-PSK maps groups of four bits to one of 16 complex constellation points uniformly spaced on the unit circle (Figure 8d). Unlike 16-QAM, which modulates both amplitude and phase, 16-PSK uses \textbf{constant-envelope} transmission --- all symbols have identical magnitude (\(|x| = 1\)) and differ only in phase:
...
\textbf{Trade-off.} The 2D classifier requires training on 16-QAM data (it cannot reuse BPSK-pretrained weights), and the number of output classes grows as \(M\) rather than \(\sqrt{M}\), which may affect convergence for very high-order constellations. For 16-QAM (\(M = 16\)), this trade-off is favourable. The experimental evaluation is presented in Section 4.17.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 1: AWGN Channel --- Theoretical vs.~Simulative BER. Single-hop, two-hop AF, and two-hop DF: closed-form theory (solid lines) versus Monte Carlo simulation (markers with 95\% CI). Theory and simulation match within the confidence interval at all SNR points.}]{results/channel_theoretical_awgn.png}}
...
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={2×2 MIMO Rayleigh --- single-hop BER with ZF, MMSE, and SIC equalization. Theoretical approximations (solid/dashed lines) overlaid with Monte Carlo simulation (markers with 95\% CI). MMSE provides \textasciitilde1--2 dB gain over ZF; SIC provides an additional \textasciitilde0.5--1 dB gain.}]{results/mimo_equalizer_comparison.png}}
\caption{2×2 MIMO Rayleigh --- single-hop BER with ZF, MMSE, and SIC equalization. Theoretical approximations (solid/dashed lines) overlaid with Monte Carlo simulation (markers with 95\% CI). MMSE provides \textasciitilde1--2 dB gain over ZF; SIC provides an additional \textasciitilde0.5--1 dB gain.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Consolidated 2×3 grid of all channel model validations. Top row: (a) AWGN, (b) Rayleigh, (c) Rician K=3 --- theory (blue) vs.~simulation (red). Bottom row: (d) all SISO channels compared, (e) fading PDFs, (f) MIMO equalizer comparison.}]{results/channel_analysis_summary.png}}
\caption{Consolidated 2×3 grid of all channel model validations. Top row: (a) AWGN, (b) Rayleigh, (c) Rician K=3 --- theory (blue) vs.~simulation (red). Bottom row: (d) all SISO channels compared, (e) fading PDFs, (f) MIMO equalizer comparison.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWGN channel --- BER vs.~SNR for all nine relay strategies with 95\% CI. AI relays outperform classical methods at low SNR; DF dominates at medium-to-high SNR.}]{results/awgn_comparison_ci.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rayleigh fading --- BER vs.~SNR for all nine relay strategies with 95\% CI.}]{results/fading_comparison.png}}
\caption{Rayleigh fading --- BER vs.~SNR for all nine relay strategies with 95\% CI.}\label{fig:fig10}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rician fading (K=3) --- BER vs.~SNR for all nine relay strategies with 95\% CI.}]{results/rician_comparison_ci.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={2×2 MIMO with ZF equalization --- BER vs.~SNR for all nine relay strategies with 95\% CI.}]{results/mimo_2x2_comparison_ci.png}}
\caption{2×2 MIMO with ZF equalization --- BER vs.~SNR for all nine relay strategies with 95\% CI.}\label{fig:fig12}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={2×2 MIMO with MMSE equalization --- BER vs.~SNR for all nine relay strategies with 95\% CI.}]{results/mimo_2x2_mmse_comparison_ci.png}}
\caption{2×2 MIMO with MMSE equalization --- BER vs.~SNR for all nine relay strategies with 95\% CI.}\label{fig:fig13}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={2×2 MIMO with MMSE-SIC equalization --- BER vs.~SNR for all nine relay strategies with 95\% CI.}]{results/mimo_2x2_sic_comparison_ci.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Normalized 3K-parameter comparison across all channels. At equal parameter budgets, all architectures converge to similar BER, with VAE being the consistent underperformer.}]{results/normalized_3k_all_channels.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Normalized 3K-parameter BER comparison on AWGN. Mamba-3K and Transformer-3K produce nearly identical BER, eliminating the gap seen at original parameter counts.}]{results/normalized_3k_awgn.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Normalized 3K-parameter BER comparison on Rayleigh fading.}]{results/normalized_3k_rayleigh.png}}
\caption{Normalized 3K-parameter BER comparison on Rayleigh fading.}\label{fig:fig17}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Normalized 3K-parameter BER comparison on Rician fading (K=3).}]{results/normalized_3k_rician_k3.png}}
\caption{Normalized 3K-parameter BER comparison on Rician fading (K=3).}\label{fig:fig18}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Complexity--performance trade-off. Training time vs.~parameter count vs.~BER improvement over DF at low SNR. The Minimal MLP (169 params) achieves the best efficiency.}]{results/complexity_comparison_all_relays.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Master BER comparison --- consolidated view of all nine relay strategies across all six channel/topology configurations.}]{results/master_ber_comparison.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={BPSK on AWGN --- all relay strategies with 95\% CI (baseline for modulation comparison).}]{results/modulation/bpsk_awgn_ci.png}}
\caption{BPSK on AWGN --- all relay strategies with 95\% CI (baseline for modulation comparison).}\label{fig:fig21}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={BPSK on Rayleigh fading --- all relay strategies with 95\% CI.}]{results/modulation/bpsk_rayleigh_ci.png}}
\caption{BPSK on Rayleigh fading --- all relay strategies with 95\% CI.}\label{fig:fig22}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={QPSK on AWGN --- BER curves closely match the BPSK baseline (Figure 21), confirming I/Q splitting validity.}]{results/modulation/qpsk_awgn_ci.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={QPSK on Rayleigh fading --- same relative ordering as BPSK, confirming hypothesis generalisability.}]{results/modulation/qpsk_rayleigh_ci.png}}
\caption{QPSK on Rayleigh fading --- same relative ordering as BPSK, confirming hypothesis generalisability.}\label{fig:fig24}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM on AWGN --- AI relays hit a BER floor near 0.22 at medium-high SNR due to \textbackslash tanh compression of multi-level signals; AF outperforms DF at low SNR (Y}]{results/modulation/qam16__awgn_ci.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM on Rayleigh fading --- wider BER gap between modulations under fading; all AI relays significantly worse than DF at every SNR point (N}]{results/modulation/qam16__rayleigh_ci.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM activation experiment (Rayleigh fading) --- same trend under fading. The improvement is significant but the gap to DF/AF remains larger than on AWGN, consistent with fading amplifying modulation-order differences (Finding 6).}]{results/qam16_activation/qam16_activation_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={BPSK constellation-aware activation comparison (AWGN). With A\_\{\textbackslash max\} = 1.0, scaled tanh reduces to standard tanh. All three bounded activations achieve equivalent BER, confirming that BPSK is insensitive to activation choice.}]{results/activation_comparison/bpsk_activation_awgn.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={BPSK constellation-aware activation comparison (Rayleigh fading). Same pattern under fading --- activation choice has negligible effect on BPSK BER.}]{results/activation_comparison/bpsk_activation_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={QPSK constellation-aware activation comparison (AWGN). With A\_\{\textbackslash max\} = 0.7071, the tighter clip range matches the binary I/Q components exactly. Sigmoid provides marginally lower BER for the Transformer relay at low SNR.}]{results/activation_comparison/qpsk_activation_awgn.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={QPSK constellation-aware activation comparison (Rayleigh fading). Similar trends under fading. The three activations remain closely matched for most relays.}]{results/activation_comparison/qpsk_activation_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM constellation-aware activation comparison (AWGN). All three bounded activations eliminate the tanh BER floor from Section 4.10, with scaled tanh and hardtanh closely matched.}]{results/activation_comparison/qam16_activation_awgn.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM constellation-aware activation comparison (Rayleigh fading). The BER floor elimination persists under fading. Sequence models benefit most from the constellation-aware bounds.}]{results/activation_comparison/qam16_activation_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Comparison of activation function shapes (left) and their derivatives (right) for A\_\{\textbackslash max\} = 0.9487 (16-QAM). Hardtanh has a sharp transition at the clip bounds; sigmoid and scaled tanh provide smooth saturation with non-zero gradients throughout.}]{results/activation_comparison/various_activation_functions.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM Rayleigh fading --- BER vs.~SNR for all 48 neural relay variants and two classical baselines (AF, DF) with 95\% confidence intervals. The plot reveals a dense cluster of neural variants between the AF and DF curves, with the best variants approaching AF performance at high SNR.}]{results/csi/csi_experiment_qam16_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Top-3 neural relay architectures compared against AF and DF for 16-QAM in Rayleigh fading. All three best-performing variants use input LayerNorm (+LN) without CSI injection.}]{results/csi/top3_qam16_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-PSK Rayleigh fading --- BER vs.~SNR for all 48 neural relay variants and two classical baselines. The neural variants form a tighter cluster than QAM16, reflecting the constant-envelope nature of PSK which reduces the amplitude-aliasing challenge.}]{results/csi/csi_experiment_psk16_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Top-3 neural relay architectures compared against AF and DF for 16-PSK in Rayleigh fading. In contrast to QAM16, all three best-performing PSK16 variants use CSI injection (+CSI or +CSI+LN).}]{results/csi/top3_psk16_rayleigh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Training loss (MSE) and accuracy for Mamba-2 (+LN scaled\_tanh). The model converges within approximately 5 epochs and stabilises, with validation accuracy closely tracking training accuracy --- indicating minimal overfitting.}]{results/csi/training_Mamba2_LN_scaled_tanh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Training loss and accuracy for Mamba S6 (+CSI tanh). The CSI-augmented input produces a smooth, monotonic training convergence, with the additional channel-state feature providing clear gradient signal for the optimizer.}]{results/csi/training_Mamba_CSI_tanh.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Training loss and accuracy for Transformer (+CSI sigmoid). The Transformer exhibits rapid initial convergence (within 3 epochs) followed by a flat plateau, characteristic of the attention mechanism's ability to capture input structure quickly.}]{results/csi/training_Transformer_CSI_sigmoid.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={16-QAM BER vs.~SNR for all relay variants (4-class and 16-class) on AWGN. The 4-class variants (dashed) plateau at BER ≈ 0.008 while the successfully trained 16-class variants (solid) continue decreasing, approaching and matching the DF baseline.}]{results/all_relays_16class/ber_all_relays_16class.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Grouped bar comparison of 4-class vs.~16-class BER at 20 dB for each architecture. The 16-class advantage is visible for MLP, VAE, Transformer, and Mamba S6. Failed variants (VAE 4-cls \textasciitilde0.50, CGAN 16-cls \textasciitilde0.50) are truncated.}]{results/all_relays_16class/grouped_bar_16class.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Heatmap of BER across all relay variants and SNR points. The 16-class variants show a clear colour transition from high BER (warm) at low SNR to near-zero BER (cool) at high SNR, while 4-class variants remain warm at high SNR due to the structural floor.}]{results/all_relays_16class/heatmap_all_relays_16class.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Top-3 16-class relay variants compared against AF and DF. VAE 16-cls, Transformer 16-cls, and MLP 16-cls all match or approach DF performance across the full SNR range.}]{results/all_relays_16class/top3_16class.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={BER vs.~SNR for E2E learned autoencoder compared to theoretical 16-QAM over Rayleigh fading. The E2E system underperforms the classical grid constellation across the full SNR range, with the gap widening at high SNR.}]{results/e2e/e2e_ber_comparison.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Learned 16-point constellation of the E2E autoencoder. The network discovers a non-rectangular geometry (resembling a hexagonal lattice or concentric APSK layout) that maximises minimum Euclidean distance under the average power constraint, unlike the classical 4 \textbackslash times 4 square grid.}]{results/e2e/e2e_constellation.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Training loss (cross-entropy) convergence of the E2E autoencoder. The model converges within approximately 200 epochs.}]{results/e2e/e2e_training_loss.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Performance comparison of the E2E autoencoder against the modular relay-based approaches from this thesis. The E2E system does not achieve lower BER than the two-hop DF relay, highlighting the limitations of the E2E approach.}]{results/e2e/e2e_relay_comparison.png}}
...
{[}1{]} J. N. Laneman, D. N. Tse, and G. W. Wornell, ``Cooperative diversity in wireless networks: Efficient protocols and outage behavior,'' \emph{IEEE Trans. Inf. Theory}, vol.~50, no. 12, pp.~3062--3080, 2004.

{[}2{]} A. Nosratinia, T. E. Hunter, and A. Hedayat, ``Cooperative communication in wireless networks,'' \emph{IEEE Commun. Mag.}, vol.~42, no. 10, pp.~74--80, 2004.

{[}3{]} T. M. Cover and A. A. El Gamal, ``Capacity theorems for the relay channel,'' \emph{IEEE Trans. Inf. Theory}, vol.~25, no. 5, pp.~572--584, 1979.

{[}4{]} A. El Gamal and Y.-H. Kim, \emph{Network Information Theory}. Cambridge University Press, 2011.

{[}5{]} B. Nazer and M. Gastpar, ``Compute-and-forward: Harnessing interference through structured codes,'' \emph{IEEE Trans. Inf. Theory}, vol.~57, no. 10, pp.~6463--6486, 2011.

{[}6{]} B. Rankov and A. Wittneben, ``Spectral efficient protocols for half-duplex fading relay channels,'' \emph{IEEE J. Sel. Areas Commun.}, vol.~25, no. 2, pp.~379--389, 2007.

{[}7{]} H. Ye, G. Y. Li, and B. H. Juang, ``Power of deep learning for channel estimation and signal detection in OFDM systems,'' \emph{IEEE Wireless Commun. Lett.}, vol.~7, no. 1, pp.~114--117, 2018.

{[}8{]} N. Samuel, T. Diskin, and G. Wunder, ``Learning to detect for MIMO systems with unknown noise statistics,'' \emph{IEEE Trans. Signal Process.}, vol.~67, no. 12, pp.~3261--3272, 2019.

{[}9{]} S. Dorner, S. Cammerer, J. Hoydis, and S. Ten Brink, ``Deep learning based communication over the air,'' \emph{IEEE J. Sel. Topics Signal Process.}, vol.~12, no. 1, pp.~132--143, 2018.

{[}10{]} H. Sun, X. Chen, Q. Shi, M. Hong, X. Fu, and N. D. Sidiropoulos, ``Learning to optimize: Training deep neural networks for interference management,'' \emph{IEEE Trans. Signal Process.}, vol.~66, no. 20, pp.~5438--5453, 2018.

{[}11{]} D. P. Kingma and M. Welling, ``Auto-encoding variational Bayes,'' in \emph{Proc. Int. Conf. Learn. Representations (ICLR)}, 2014.

{[}12{]} M. Mirza and S. Osindero, ``Conditional generative adversarial nets,'' \emph{arXiv preprint arXiv:1411.1784}, 2014.

{[}13{]} I. Gulrajani, F. Ahmed, M. Arjovsky, V. Dumoulin, and A. Courville, ``Improved training of Wasserstein GANs,'' in \emph{Advances in Neural Information Processing Systems}, vol.~30, 2017.

{[}14{]} I. Goodfellow, J. Pouget-Abadie, M. Mirza, B. Xu, D. Warde-Farley, S. Ozair, A. Courville, and Y. Bengio, ``Generative adversarial nets,'' in \emph{Advances in Neural Information Processing Systems}, vol.~27, 2014.

{[}15{]} A. Vaswani, N. Shazeer, N. Parmar, J. Uszkoreit, L. Jones, A. N. Gomez, Ł. Kaiser, and I. Polosukhin, ``Attention is all you need,'' in \emph{Advances in Neural Information Processing Systems}, vol.~30, 2017.

{[}16{]} A. Gu and T. Dao, ``Mamba: Linear-time sequence modeling with selective state spaces,'' \emph{arXiv preprint arXiv:2312.00752}, 2024.

{[}17{]} A. Gu, K. Goel, and C. Ré, ``Efficiently modeling long sequences with structured state spaces,'' in \emph{Proc. Int. Conf. Learn. Representations (ICLR)}, 2022.

{[}18{]} T. Dao and A. Gu, ``Transformers are SSMs: Generalized models and efficient algorithms through structured state space duality,'' \emph{arXiv preprint arXiv:2405.21060}, 2024.

{[}19{]} D. Tse and P. Viswanath, \emph{Fundamentals of Wireless Communications}. Cambridge University Press, 2005.

{[}20{]} P. W. Wolniansky, G. J. Foschini, G. D. Golden, and R. A. Valenzuela, ``V-BLAST: An architecture for realizing very high data rates over the rich-scattering wireless channel,'' in \emph{Proc. IEEE ISSSE}, pp.~295--300, 1998.

{[}21{]} J. G. Proakis and M. Salehi, \emph{Digital Communications}, 5th ed.~McGraw-Hill, 2008.

{[}22{]} M. K. Simon and M.-S. Alouini, \emph{Digital Communication over Fading Channels}, 2nd ed.~Wiley, 2005.

{[}23{]} I. Goodfellow, Y. Bengio, and A. Courville, \emph{Deep Learning}. MIT Press, 2016.

{[}24{]} B. Sklar, \emph{Digital Communications: Fundamentals and Applications}, 2nd ed.~Prentice Hall, 2001.

{[}25{]} R. S. Sutton and A. G. Barto, \emph{Reinforcement Learning: An Introduction}, 2nd ed.~MIT Press, 2018.

{[}26{]} I. E. Telatar, ``Capacity of multi-antenna Gaussian channels,'' \emph{European Trans. Telecommun.}, vol.~10, no. 6, pp.~585--595, 1999.

{[}27{]} G. J. Foschini, ``Layered space-time architecture for wireless communication in a fading environment when using multi-element antennas,'' \emph{Bell Labs Tech. J.}, vol.~1, no. 2, pp.~41--59, 1996.

{[}28{]} L. Zheng and D. N. C. Tse, ``Diversity and multiplexing: A fundamental tradeoff in multiple-antenna channels,'' \emph{IEEE Trans. Inf. Theory}, vol.~49, no. 5, pp.~1073--1096, 2003.

{[}29{]} D. N. C. Tse and S. V. Hanly, ``Linear multiuser receivers: Effective interference, effective bandwidth and user capacity,'' \emph{IEEE Trans. Inf. Theory}, vol.~45, no. 2, pp.~641--657, 1999.

{[}30{]} S. Loyka and F. Gagnon, ``Performance analysis of the V-BLAST algorithm: An analytical approach,'' \emph{IEEE Trans. Wireless Commun.}, vol.~3, no. 4, pp.~1326--1337, 2004.

{[}31{]} I. Higgins, L. Matthey, A. Pal, C. Burgess, X. Glorot, M. Botvinick, S. Mohamed, and A. Lerchner, ``β-VAE: Learning basic visual concepts with a constrained variational framework,'' in \emph{Proc. Int. Conf. Learn. Representations (ICLR)}, 2017.

{[}32{]} K. He, X. Zhang, S. Ren, and J. Sun, ``Delving deep into rectifiers: Surpassing human-level performance on ImageNet classification,'' in \emph{Proc. IEEE Int. Conf. Comput. Vision (ICCV)}, pp.~1026--1034, 2015.
...
\chapter*{Abstract (Hebrew)}\label{sec:abstract-hebrew}
...
\chapter*{Abstract (English)}\label{sec:abstract-english}
