...
\REV{Fixes applied. (Finding \#1) \texttt{main.tex} previously included the nonexistent \texttt{chapters/ch00\_frontmatter}, so this abstract was silently absent from the compiled PDF; the include now points at \texttt{chapters/frontmatter.tex}. (Finding \#3) The former blanket claim ``All results use \ldots 100,000 bits per SNR point, 10 trials'' was inaccurate --- E1 uses $20\times50$k and Chapter~7 uses $5\times50$k --- so it now scopes that budget to the canonical comparison and points to Section~\ref{sec:monte-carlo-ber-estimation}, where the per-experiment budgets are now listed.}
