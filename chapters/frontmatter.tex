...
\REV{Fix applied (review finding \#1): \texttt{main.tex} previously included the nonexistent \texttt{chapters/ch00\_frontmatter}, so this abstract was silently absent from the compiled PDF; the include now points at \texttt{chapters/frontmatter.tex} and the abstract renders again. One residual item from the review still applies to the text below: the blanket claim ``All results use \ldots 100,000 bits per SNR point, 10 trials'' does not hold for E1 ($20\times50$k) or Chapter~7 ($5\times50$k) --- reword once the per-experiment budget table suggested in Section~4.3.1 exists.}
