...
\caption{Flat unknown channels: classical relay vs.\ MLP-169 BER at 8/12/16/20 dB (mean, 5 trials). The maximum DF--MLP gap over all cases and SNRs is $0.0036$, within statistical noise. Unknown parameters alone do not defeat classical relays; memory does.}\label{tbl:tableE6flat}\tabularnewline
\toprule\noalign{}
...
Unknown ISI $\to$ AWGN & AF & 0.2044 & 0.1779 & 0.1872 & 0.2141 \\
 ...
 & \textbf{MLP (169p)} & \textbf{0.0421} & \textbf{0.0030} & \textbf{$<$5e-5} & \textbf{$<$5e-5} \\
...
Unknown ISI $\to$ Rayleigh & AF & 0.2331 & 0.1985 & 0.1933 & 0.2063 \\
 ...
 & \textbf{MLP (169p)} & \textbf{0.0945} & \textbf{0.0326} & \textbf{0.0125} & \textbf{0.0049} \\
...
Control: canonical Rayleigh & AF & 0.1591 & 0.0894 & 0.0484 & 0.0236 \\
 ...
 & MLP (169p) & 0.1202 & 0.0580 & 0.0265 & 0.0115 \\
\end{longtable}
...
\caption{Pilot-budget sweep at 10~dB. Pilot-aided Viterbi outperforms the pilot-free MLP down to approximately 10 pilots per block, but collapses at 5 pilots when the least-squares channel estimate becomes too noisy to construct a reliable trellis ($0.119 \pm 0.084$). In contrast, the MLP remains essentially unchanged because it operates with an amortized posterior and requires no pilots.}
\label{fig:e6_partial_pilot_budget} 
\end{figure} 
\begin{figure}[H] 
\centering 
\includegraphics[width=0.85\linewidth]{results/e6_partial_short_blocks_overhead.png} 
\caption{Block-length sweep with 10 pilots per block. Payload BER remains similar across methods, but the effective pilot overhead of the classical approach increases rapidly as the block length decreases, growing from approximately 1\% at $L=1000$ to 25\% at $L=40$. The pilot-free MLP incurs no such overhead and therefore becomes increasingly attractive for short packets.}
