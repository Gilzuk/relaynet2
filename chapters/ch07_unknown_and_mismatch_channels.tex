...
Unknown ISI $\to$ AWGN & AF & 0.2044 & 0.1779 & 0.1872 & 0.2141 \\
 ...
 & \textbf{MLP (169p)} & \textbf{0.0421} & \textbf{0.0030} & \textbf{$<5\times10^{-5}$} & \textbf{$<5\times10^{-5}$} \\
...
\caption{Pilot-budget sweep at 10~dB (mean over 6 trials, 40{,}000 bits/point). Pilot-aided Viterbi outperforms the pilot-free MLP down to approximately 10 pilots per block, but collapses at 5 pilots when the least-squares channel estimate becomes too noisy to construct a reliable trellis ($0.119 \pm 0.084$). In contrast, the MLP remains essentially unchanged because it operates with an amortized posterior and requires no pilots.}
