...
\end{equation}\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad df-high-snr-two-hop-penalty}, i.e., the two-hop penalty is approximately a factor of 2 in BER.
...
\REV{Fix applied (scope and duplication): Chapter~2 is now a literature review only, with no experimental results in it. Removed: (i)~the ``Channel Model Validation (Simulative)'' subsection, whose validation results duplicated the E1 experiment reported in Section~\ref{sec:channel-model-validation} (its theoretical-SNR table moved there); (ii)~the ``Fading Coefficient Distributions'' subsection, which analysed Rician $K$-factors although Rician fading is future work and no Rician result appears in the thesis; (iii)~two channel-validation figures that were printed identically in both chapters, so each figure now appears exactly once; and (iv)~the ``Relay methods comparison'' positioning table, which duplicated the research-positioning table in Chapter~\ref{sec:research-objectives}.}

\REV{Fix applied (scope, per supervisor comments 2, 4 and 6 in Appendix~E): the ``Theoretical Foundations: MIMO Equalization'' section (ZF / LMMSE / LMMSE-SIC for a $2\times2$ topology) has been \emph{removed}. The MIMO thread was deferred to future work in the canonical restructure and no MIMO experiment appears anywhere in this thesis, so the section described machinery the document never uses; Appendix~E already listed it among the removed material. The equalizer terminology that the section carried is therefore moot, and the term ``MMSE'' now appears in this thesis only in its correct non-linear sense (the MMSE-optimal estimator $\mathbb{E}[x\mid y]$).}
