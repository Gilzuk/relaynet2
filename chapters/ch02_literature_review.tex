...
\AK{This is a different relay-channel model where, in addition to the two-hop link Source$\to$Relay$\to$Destination, there is a direct Relay$\to$Destination link. In the absence of a direct link, the capacity of this problem is trivial and equals to the minimum of the individual links: $C = \min\{C_\mathrm{src \to rel}, C_{rel \to dest} \}$. However, to attain this capacity, a large delay must be introduced. The question then becomes how to reliably communicate over a relay-channel with low latency.}

 The theoretical analysis of relay channels originates from van der Meulen’s 1971 formulation and the landmark capacity bounds of Cover and El Gamal \cite{CoverGamal1979RelayChannel}, who derived inner and outer bounds for the general relay channel that still represent the tightest known results for many scenarios. In a two-hop relay network, a source sends a signal to a relay, which processes the signal and forwards it to the destination. This type of architecture underpins standards such as LTE-Advanced and 5G NR, where relay nodes are used to fill coverage holes and improve performance for users at the cell edge \cite{Laneman2004CooperativeDiversity}, \cite{Nosratinia2004CooperativeCommunication}

\subsection{Information-Theoretic Foundations}\label{sec:information-theoretic-foundations}

The three-terminal relay channel consists of a source \(S\), a relay \(R\), and a destination \(D\). The capacity of this channel depends critically on the relay processing strategy \(f(\cdot)\) and the channel statistics. Cover and El Gamal \cite{CoverGamal1979RelayChannel} established two fundamental capacity bounds:
...
}\end{equation} \par{\small\textit{source: [\cite{CoverGamal1979RelayChannel}, Thm. 1], [\cite{ElGamalKim2011NetworkIT}, Ch. 16]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad relay-capacity-bound}
...
\textbf{DF achievability bound.} Under decode-and-forward, the relay fully decodes the source message and cooperates with the source to transmit a common message to the destination at the rate  shown in \small\textit{\cite{CoverGamal1979RelayChannel}, Thm. 1
}:

\begin{equation}\protect\phantomsection\label{eq:df-capacity}{C_{\text{DF}} = \max_{p(x, x_R)} \min\left\{I(X; Y_R | X_R), \; I(X, X_R; Y_D)\right\}
}\end{equation} 
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad df-capacity}

This rate is achievable using block Markov encoding at the source and backward decoding at the destination. 
The first mutual information term represents the relay decoding constraint, requiring the relay to fully decode the source message, while the second term corresponds to the destination decoding constraint.

Decode-and-forward (DF) achieves the cut-set bound when the source–relay link is stronger than the relay–destination link. 
In this regime, DF is capacity-optimal, which typically occurs when the relay is located close to the source.

For the degraded relay channel—where the relay’s observation is a degraded version of the destination’s observation, or vice-versa: Cover and El Gamal showed that the DF bound coincides with the cut-set bound, thereby establishing the exact channel capacity.
In the Gaussian relay channel with equal per-hop signal-to-noise ratio (SNR) $\gamma$, the resulting two-hop DF capacity is given by:
\begin{equation}
C_{\text{DF}}^{\text{Gaussian}} = \frac{1}{2}\log_2\left(1 + \gamma\right),
\label{eq:df-gaussian-capacity}
\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad df-gaussian-capacity}

which is identical to the single-hop Gaussian channel capacity.
Consequently, when the source–relay link is sufficiently strong, the relay introduces no rate penalty \small\textit{Source: \cite[Eq.~(7)]{Laneman2004CooperativeDiversity}, \cite{CoverGamal1979RelayChannel}}.

The half-duplex constraint introduces a spectral efficiency penalty of factor 2 (since two time slots are needed per symbol), but this is a common assumption in practical relay standards and simplifies the analysis while remaining representative of many practical relay-processing scenarios.
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Sec. II]}} \AK{unusual citation.}
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Sec. II]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad awgn-hop2}

where \(x \in \{-1, +1\}\) is the transmitted BPSK symbol, \(y_R\) is the signal received at the relay, \(x_R = f(y_R)\) is the relay's output after processing \AK{this is the transmitted signal by the relay}, and \(y_D\) is the signal received at the destination. The noise terms are independent and identically distributed with variance 
...
\AK{The time axis is missing. Aggregation of information can be useful, especially when incorporating an error correction code. Do you allow only scalar operations at the relay?}

%---------------------------------------------------------
\subsection{Cooperative Diversity and Practical Relevance}\label{sec:cooperative-diversity-and-practical-relevance}

Laneman, Tse, and Wornell \cite{Laneman2004CooperativeDiversity} showed that cooperative relaying achieves spatial diversity without requiring multiple antennas at any single node. Specifically, a system with \(L\) cooperating single-antenna relays can achieve a diversity order of \(L + 1\), meaning the outage probability decays as \(\text{SNR}^{-(L+1)}\). For a single relay (\(L=1\)), this yields second-order diversity: a significant improvement over the first-order diversity of point-to-point communication over fading channels. \AK{You do not assume fading channels and hence diversity is irrelevant in your model. Moreover, even if the two channels were fading channels, the diversity order would not be 2.}

In practical deployments, relay nodes serve several roles: 
\begin{itemize}
    \item \textbf{Range extension} for cell-edge users where the direct link is too weak, 
    \item \textbf{Coverage filling} in shadowed areas behind buildings or terrain.
    \item \textbf{Capacity enhancement} through spatial reuse.
    \item \textbf{Energy efficiency} by reducing transmission power requirements through shorter per-hop distances. 
\end{itemize}
    The 3GPP LTE-Advanced standard (Release 10+) defines Type I (non-transparent) and Type II (transparent) relay nodes, while 5G NR introduces integrated access and backhaul (IAB) nodes that extend this concept to millimeter-wave and sub-THz bands. The AI-based relay processing studied in this thesis is applicable to any of these deployment scenarios, as the neural network operates at the baseband processing level independently of the RF front-end and protocol layer.
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Eq. 1]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Eq. 2]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Sec. III]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Eq. 9], [\cite{ProakisSalehi2008DigitalComm}, Ch. 14]}}
...
Beyond AF and DF, several additional relay strategies have been proposed in the literature. 
\begin{enumerate}
    \item \textbf{Compress-and-Forward (CF)} has the relay quantize and compress the received signal using Wyner-Ziv source coding, then forward the compressed representation to the destination, which exploits its own direct-link observation as side information to decode \cite{CoverGamal1979RelayChannel}, \cite{ElGamalKim2011NetworkIT}. 
    \item \textbf{Compute-and-Forward (CoF)} leverages lattice codes to allow relays to decode \emph{linear combinations} of transmitted messages rather than individual codewords; the destination then solves a system of linear equations to recover the original messages \cite{NazerGastpar2011ComputeForward}. 
    \item \textbf{Estimate-and-Forward (EF)} computes an MMSE estimate of the transmitted signal at the relay and forwards the soft estimate, avoiding the hard-decision errors of DF while providing a more structured output than AF.
    \item \textbf{Selective and incremental relaying} are hybrid protocols: in selective relaying the relay forwards only when it can decode correctly, while in incremental relaying the relay transmits only when the destination signals (via a feedback link) that the direct transmission has failed \cite{Laneman2004CooperativeDiversity}. 
    \item \textbf{Two-way relaying with network coding} allows both endpoints to transmit simultaneously to the relay, which combines the received signals (e.g., via XOR or lattice coding) and broadcasts the result back, effectively doubling spectral efficiency \cite{RankovWittneben2007HalfDuplexRelay}. 
    \item \textbf{Filter-and-Forward (FF)} applies a linear filter (e.g., matched filter or Wiener filter) at the relay before forwarding, providing a structured intermediate between AF's simple scaling and DF's full regeneration.
\end{enumerate}
...
\AK{You haven't defined the system model so this discussion seems out of place.}

Multiple-input multiple-output (MIMO) systems employ multiple antennas at both transmitter and receiver to exploit spatial multiplexing and diversity gains \cite{TseViswanath2005Fundamentals}. The theoretical foundation was established by Telatar \cite{Telatar1999MIMO} and Foschini \cite{Foschini1996LayeredST}, who independently showed that the ergodic capacity of an \(N_t \times N_r\) MIMO channel with independent Rayleigh fading scales as: (Equation~\ref{eq:af-gain})
\AK{The relevance to this equtaion is not clear. Also, what do you mean by scaling? Isn't that the capacity for any SNR? Also, you need to specify whatr }
...
}\end{equation} \par{\small\textit{source: [\cite{Telatar1999MIMO}, Eq. 1],[\cite{Foschini1996LayeredST}, Eq. 1]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-capacity}
...
\AK{Here you introduce fading for the first. You could've done that already in the SISO case. Also, now the channels are complex whereas up until now they were real (and with fixed known gains).}
...
}\end{equation} \par{\small\textit{source: [\cite{TseViswanath2005Fundamentals}, Ch. 7.2]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-received}

where \(\mathbf{H} \in \mathbb{C}^{2 \times 2}\) is the channel matrix with \(H_{ij} \sim \mathcal{CN}(0, 1)\) (independent Rayleigh fading per link), \(\mathbf{x} \in \mathbb{C}^{2}\) is the transmitted symbol vector with
\begin{equation}\protect\phantomsection\label{eq:mimo-tx-symbols}{
\mathbb{E}[\mathbf{x}\mathbf{x}^H] = \frac{P}{N_t}\mathbf{I}} 
\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-tx-symbols}
and noise
\begin{equation}\protect\phantomsection\label{eq:mimo-noise-per-antenna}{\mathbf{n} \sim \mathcal{CN}(\mathbf{0}, \sigma^2\mathbf{I})}
\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-noise-per-antenna}
The per-antenna SNR is 
\begin{equation}\protect\phantomsection\label{eq:mimo-snr}{\text{SNR} = \frac{P}{N_t \sigma^2}}
\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-snr}
The fundamental trade-off in MIMO systems is between spatial multiplexing gain (transmitting multiple independent streams) and diversity gain (transmitting redundant copies to combat fading). Zheng and Tse \cite{ZhengTse2003DMT} formalized this as the diversity-multiplexing tradeoff (DMT): for an \(N_t \times N_r\) system at multiplexing gain \(r\), the maximum achievable diversity order is 
\begin{equation}\protect\phantomsection\label{eq:mimo-diversity-order}{d(r) = (N_t - r)(N_r - r)}. 
\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-diversity-order}

In the \(2 \times 2\) case at full spatial multiplexing (\(r = 2\)), \(d = 0\) : meaning no diversity protection and the system is interference-limited. This makes the choice of the equalization method critical.

\AK{The diversity--multiplexing tradeoff (DMT) is irrelevant in the ergodic setting. DMT assumes slow fading and disucsses the performance in the presence of some outage probability.}
...
}\end{equation} \par{\small\textit{source: [\cite{TseViswanath2005Fundamentals}, Ch. 7.2],[\cite{TseHanly1999LinearMultiuser}]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad zf-equalizer}
...
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mmse-equalizer}
...
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mmse-sinr}

At low SNR, MMSE significantly outperforms ZF because it avoids noise amplification; at high SNR (\(\sigma^2 \to 0\)), the MMSE filter converges to the ZF solution. Crucially, MMSE achieves the same diversity order as ZF (\(d = 1\)) but with a superior coding gain, meaning it provides a constant SNR advantage across the entire operating range \cite{TseHanly1999LinearMultiuser}.

\textbf{Successive Interference Cancellation (SIC).} SIC is a non-linear detection technique that exploits the layered structure of spatial multiplexing. The MMSE-ordered V-BLAST algorithm \cite{Wolniansky1998VBLAST}, \cite{Foschini1996LayeredST} proceeds as follows:
...
}\end{equation} \par{\small\textit{source: [\cite{GoodfellowBengioCourville2016DeepLearning}, Ch. 5]}}
...
}\end{equation} \par{\small\textit{source: [\cite{GoodfellowBengioCourville2016DeepLearning}, Ch. 5, Eq. 5.18]}}
...
}\end{equation} \par{\small\textit{source: [\cite{He2015DelvingRectifiers}, Ch. 6]}}
...
}\end{equation} \par{\small\textit{source: [\cite{KingmaWelling2014VAE}, Eq. 2]}}
...
}\end{equation} \par{\small\textit{source: [\cite{KingmaWelling2014VAE}, Eq. 3]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Goodfellow2014GAN}, Eq. 1]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Gulrajani2017WGANgp}, Eq. 1]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Gulrajani2017WGANgp}, Eq. 3]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Gulrajani2017WGANgp}, Eq. 2]}}
...
\textbf{Transformers} \cite{Goodfellow2014GAN} use multi-head self-attention to capture global dependencies in sequences. The core attention mechanism computes a weighted combination of value vectors, where the weights are derived from the compatibility of query and key vectors:
...
}\end{equation} \par{\small\textit{source: [\cite{Vaswani2017Attention}, Eq. 1]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Vaswani2017Attention}, Eq. 2]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Vaswani2017Attention}, Sec. 3.5]}}
...
}\end{equation} \par{\small\textit{source: [\cite{GuGoelRe2022SSM}, Eq. 1]}}
...
}\end{equation} \par{\small\textit{source: [\cite{GuGoelRe2022SSM}, Eq. 2]}}
...
}\end{equation} \par{\small\textit{source: [\cite{GuGoelRe2022SSM}, Eq. 3]}}
...
\textbf{Mamba} \cite{GuDao2024Mamba} extends the SSM framework by making the parameters \textbf{input-dependent} (selective), transforming the LTI system into a linear time-varying (LTV) system:
...
}\end{equation} \par{\small\textit{source: [\cite{GuDao2024Mamba}, Eq. 4]}}
...
}\end{equation} \par{\small\textit{source: [\cite{DaoGu2024TransformersSSM}, Eq. 2]}}
...
}\end{equation} \par{\small\textit{source: [\cite{DaoGu2024TransformersSSM}, Eq. 3]}}
...
}\end{equation} \par{\small\textit{source: [\cite{ProakisSalehi2008DigitalComm}, Ch. 4]}}
...
}\end{equation} \par{\small\textit{source: [\cite{ProakisSalehi2008DigitalComm}, Ch. 4, Eq. 4-3-13]}}
...
}\end{equation} \par{\small\textit{source: [\cite{Laneman2004CooperativeDiversity}, Eq. 9]}}
...
}\end{equation} \par{\small\textit{source: [\cite{ProakisSalehi2008DigitalComm}, Ch. 14]}}
...
}\end{equation} \par{\small\textit{source: [\cite{ProakisSalehi2008DigitalComm}, Ch. 14, Eq. 14-4-1]}}
...
}\end{equation} \par{\small\textit{source: [\cite{ProakisSalehi2008DigitalComm}, Eq. 14-4-15]}}
...
}\end{equation} \par{\small\textit{source: [\cite{SimonAlouini2005FadingChannels}, Ch. 2]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad rician-channel}
...
}\end{equation} \par{\small\textit{source: [\cite{SimonAlouini2005FadingChannels}, Ch. 2, Eq. 2.6]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad rician-pdf}
...
\subsubsection{Single-hop BER}\label{sec:Rician-Single-hop-ber}
\textbf{Single-hop BER.} The average BER for BPSK over a Rician channel is obtained via the moment-generating function (MGF) approach \cite{SimonAlouini2005FadingChannels}: (Equation~\ref{eq:af-gain})
...
}\end{equation} \par{\small\textit{source: [\cite{SimonAlouini2005FadingChannels}, Ch. 8, Eq. 8.98]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad rician-ber-mgf}
...
}\end{equation} \par{\small\textit{source: [\cite{SimonAlouini2005FadingChannels}, Ch. 8, Eq. 8.99]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad rician-mgf}
...
\subsubsection{Two-hop DF BER}\label{sec:Rician-two-hop-df-ber}
As with other channels, \(P_e^{\text{DF,Rician}} = 2P_e^{\text{Rician}}(1 - P_e^{\text{Rician}})\).
...
\subsection{The $2 \times 2$ MIMO Channel : Theoretical Analysis and equalization methods}\label{sec:mimo-channel-theoretical-analysis}
...
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-received-2x2}

The theoretical per-stream BER depends on the equalization technique as shown in the following sub-sections.

\subsubsection{ZF equalization}\label{sec:zf-equalization}
After ZF equalization (\(\hat{\mathbf{x}} = \mathbf{H}^{-1}\mathbf{y}\)), the effective noise on stream \(k\) has variance \(\sigma^2 [\mathbf{H}^{-1}(\mathbf{H}^{-1})^H]_{kk}\). For a 2×2 system with i.i.d. Rayleigh fading, each post-ZF stream sees an effective diversity order of \(n_R - n_T + 1 = 1\), identical to SISO Rayleigh. Therefore:
...
}\end{equation} \par{\small\textit{source: [\cite{TseViswanath2005Fundamentals}, Ch. 7.2],[\cite{ZhengTse2003DMT}]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad zf-ber-mimo}
...
\subsubsection{MMSE equalization}\label{sec:MMSE-equalization}
...
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mmse-ber-mimo}
