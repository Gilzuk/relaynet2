...
\REV{Two observations on Table~\ref{tbl:table2}. (1)~Credit where due: the DF value at 20~dB (0.0047) matches the closed form $2P(1-P)$ with $P = \frac{1}{2}(1-\sqrt{100/101}) \approx 0.0025$ to two significant figures --- a quiet but convincing end-to-end validation of the simulator against Section~2.5.2. (2)~The VAE column is not merely an ``underperformer'': it is pinned at $0.25$--$0.40$ at \emph{all} SNRs, i.e.\ it never learns the task at any operating point. Before attributing this to the generative paradigm (as Section~2.3.3 does), rule out the obvious implementation cause: if inference samples $\mathbf{z}\sim q_\phi$ stochastically, replacing the sample with the posterior mean $\boldsymbol{\mu}$ at test time is standard practice and may recover most of the gap. Either report the deterministic-decoding variant or soften the paradigm-level conclusion --- as it stands, a single hyperparameter choice may be doing the work of an architectural claim.}
...
\REV{Fix applied (review finding \#2): the former Table~16 (``16-QAM BER improvements \ldots with and without CSI injection and layer normalisation'') has been removed. It reported a ``Mamba S6 (+CSI + LN)'' variant, but the CSI-injection study was deliberately cut in the restructure (see Appendix~E / \texttt{ak\_response\_appendix}) and is listed as future work in Chapter~1's scope table, with no methods section defining it. The 16-QAM story is carried by the joint 2D-classification study (Section~\ref{sec:class-2d-classification-for-qam16}), which is the cleaner, in-scope result. The orphaned Conclusions 8--10 that cited this table (plus the 48-variant search and 16-PSK) have been removed from Chapter~\ref{sec:discussion-and-conclusions} accordingly.}
...
\end{figure}
\REV{Fix applied: the Figure~\ref{fig:fig25} caption was truncated mid-sentence (``\ldots at low SNR (Y'') --- the dangling fragment has been removed and the sentence closed. The Figure~\ref{fig:fig50} mojibake is fixed at its own caption below.}
...
\REV{This analytic floor derivation is the right way to open the chapter --- I verified the four normalized slicer amplitudes ($\{1.67, 0.91, 0.61, -0.15\}$ for $[1,0.7,0.5]/\sqrt{1.74}$) and the $0.25$ limit; the prediction is correct, and anchoring the empirical failure to a closed-form mechanism (rather than just showing curves) is what makes H6 credible. One editorial nit in the Trials list above: the enumeration runs (i), (ii), (iv), (v) --- item (iii) is missing.}
...
\REV{This flat-channel control is the methodological highlight of the thesis: it pre-empts the strongest skeptical reading (``the classical baselines failed because the channel was unknown, not because they were the wrong model class'') with a designed falsification test, and reports a null result honestly. Very few learning-for-communications papers include this control; keep it, and consider citing the maximum-gap number with its trial budget so the ``within statistical noise'' claim is checkable.}
...
\REV{Sharpen the ``identifiability'' language: with 5 pilot observations and 3 complex taps the LS problem is formally \emph{overdetermined} (5 equations, 3 unknowns), so the channel is not strictly unidentifiable --- what collapses is the \emph{estimate variance}: at 10~dB the LS error with $N_p=5$ is large enough that MLSE runs on a badly wrong trellis, and occasional catastrophic fits dominate the average (consistent with the very wide CI at that point). Phrase it as an estimation-variance cliff (``too few pilots to average the noise, so the LS estimate is unreliable'') rather than a rank/identifiability threshold; the empirical finding is unchanged and becomes unimpeachable. Also state the CI at the 5-pilot point in the text --- the mean $0.119$ alone hides that individual blocks range from fine to catastrophic, which is itself part of the story.}
...
\REV{``The only option that remains reliable'' slightly overreaches: blind CMA is also pilot-free, and this sweep never tests CMA at $L=40$ (where its adaptation loop plausibly fails to converge within the block --- which would \emph{support} the claim). Either add CMA to panel~(b) or qualify to ``the only \emph{one-shot} option'': the distinctive property demonstrated here is zero per-block adaptation, not uniqueness among pilot-free methods.}
...
\REV{The $30$--$90\times$ number needs an implementation caveat that the ``architectural'' sentence currently papers over: the measurement compares a \emph{vectorized NumPy} matmul against an \emph{interpreted Python} trellis loop, so a large (perhaps dominant) share of the ratio is interpreter overhead, not the sequential data dependency itself --- a C or SIMD Viterbi would narrow the wall-clock gap substantially even though the per-symbol dependency is real. The scaling argument in the preceding paragraph ($M^{L}$ vs.\ constant) is the robust, implementation-independent claim; state the wall-clock ratio as ``for our reference implementations'' and let the scaling carry the conclusion. The chapter is otherwise commendably honest about Viterbi being cheaper at BPSK/$L{=}3$ (64 vs.\ $\sim$330 ops) --- extend the same honesty to the timing comparison.}
