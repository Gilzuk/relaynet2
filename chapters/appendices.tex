...
\item \textbf{MIMO equalization section implied unreported experiments}
(Section~\ref{sec:theoretical-foundations-mimo-equalization}). The section stated
that three equalizers ``were implemented,'' with speed-up figures, although MIMO
is future work and no MIMO result appears in the thesis. \emph{Resolved:} the
section is reframed explicitly as background, the implementation note is moved to
Appendix~C, and the terminology is corrected from MMSE to LMMSE.
