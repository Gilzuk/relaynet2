...
Source bits are BPSK-modulated, passed through Hop 1 (SISO channel), processed by the relay (classical or neural), then transmitted over Hop 2 (SISO or 2×2 MIMO Rayleigh channel) and equalized at the destination for imperfect CSI cases.
...
Four modulation schemes are supported. 
The QSK, 16-QAM and 16-PSK modulations are evaluated in Sections \ref{sec:higher-order-modulation-scalability-constellation-aware-training} and \ref{sec:input-normalization-and-csi-injection} to test whether the BPSK findings generalize to complex constellations. 
\begin{itemize}
    \item The primary experiments use Binary Phase-Shift Keying (BPSK): bits \(b \in \{0, 1\}\) are mapped to real symbols \(x = 1 - 2b \in \{-1, +1\}\).
    \item Extensions to Quadrature Phase-Shift Keying (QPSK). QPSK maps pairs of bits to complex symbols on the unit circle (2 bits/symbol).
    \item 16-point Quadrature Amplitude Modulation (16-QAM) 16-QAM maps groups of four bits to a \(4 \times 4\) Gray-coded grid (4 bits/symbol)
    \item 16-point Phase-Shift Keying (16-PSK) . 16-PSK maps groups of four bits to 16 equally spaced points on the unit circle (4 bits/symbol).
\end{itemize}

Full modulation details are given in Section \ref{sec:modulation-schemes}.


\subsection{Hop Model}\label{sec:hop-model}
Each hop applies a channel function followed by optional equalization: (Equation~\ref{eq:qpsk-mapping})
...
\node[box,   right=1.1cm of eq]  (demod)    {BPSK\\Demod.};
\node[src,  right=1.1cm of demod]   (dst)   {Dest.\\bits};
\draw[arr] (src)   -- node[above,font=\small]{bits} (mod);
\draw[arr] (mod)   -- node[above,font=\small]{$x$} (ch1);
\draw[arr] (ch1)   -- node[above,font=\small]{$y_R$} (relay);
\draw[arr] (relay) -- node[above,font=\small]{$\hat{x}_R$} (ch2);
\draw[arr] (ch2)   -- node[above,font=\small]{$\mathbf{y}$} (eq);
\draw[arr] (eq)    -- (demod);
\draw[arr] (demod)    -- node[above,font=\small]{bits} (dst);
\end{tikzpicture}}
...
The relay's neural network solves a \emph{denoising} problem (Hop 1 noise removal); the MIMO equalizer solves an \emph{interference cancellation} problem (Hop 2 stream separation). The two stages are independent, and their gains are additive. Each antenna at the relay receives, as shown in eq \ref{eq:relay-received-siso}:
...
\subsubsection{Relay's nueral networks}
...
\subsubsection{MIMO 2x2 channel }

The MIMO topology applies to Hop 2, where the relay retransmits using 2 TX antennas, and the destination has 2 RX antennas. Each of the 4 TX-RX antenna pairs experiences an independent Rayleigh fading channel (\(H_{ij} \sim \mathcal{CN}(0,1)\)), creating \textit{inter-stream interference}:
...
\end{aligned}
}\end{equation} \par{\small\textit{source: [\cite{ZhengTse2003DMT}, Ch. 7.2]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-interference}

this can be represented in the matrix form \ref{eq:mimo-interference-matrix}:
\begin{equation}\protect\phantomsection\label{eq:mimo-interference-matrix}{\mathbf{Y} = \mathbf{H}\,\cdot \mathbf{X}_R + \mathbf{N}}
\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-interference-matrix}

where, 
\begin{equation}\protect\phantomsection\label{eq:mimo-interference-matrix-elements}{\mathbf{Y} \triangleq
\begin{bmatrix}
y_1\\
y_2
\end{bmatrix},\quad
\mathbf{X}_R \triangleq
\begin{bmatrix}
x_{R,1}\\
x_{R,2}
\end{bmatrix},\quad
...
h_{21} & h_{22}
\end{bmatrix}}
\end{equation}\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mimo-interference-matrix-elements}
...
\subsubsection{MIMO equalizer (ZF, MMSE, SIC) }
The Three equalizer options explored in section \ref{sec:equalization-methods} show a trade-off between complexity and  performance. \emph{ZF}, on one hand, removes all interference, but on the other hand, amplifies noise. \emph{MMSE} adds regularization \(\sigma^2\mathbf{I}\) hence trades residual interference for less noise amplification. Finally, \emph{SIC}, which detects the strongest stream first via MMSE, cancels its contribution, than detects the remaining stream interference-free. This \textbf{non-linear} technique eliminates interference sequentially.

Improvements in relay processing and in MIMO equalization contribute \textbf{additively and independently}. Enhanced relay denoising reduces the noise forwarded into Hop-2, whereas enhanced equalization improves separation of the spatially multiplexed streams. Combining the strongest relay with the strongest equalizer therefore achieves the minimum BER, consistent with the results in Section \ref{sec:mimo-2x2-bpsk-performance}
...
\begin{enumerate}
\item \textbf{Cassical AF:} Gain amplification as shown in equation \ref{eq:af-gain}.  AF serves as the lower baseline. it performs no intelligent processing and simply rescales the received signal, preserving both signal and noise. Its theoretical BER is given in Section \ref{eq:af-snr}.
\item \textbf{Classical DF:} Demodulates, recovers bits and re-modulates clean BPSK symbols. DF serves as the upper classical baseline: it performs the maximum possible classical processing (full signal regeneration). At high SNR, DF approaches error-free relay operation. Its theoretical BER follows the error-composition formula \ref{eq:equal-hop-snr}).

\item \textbf{Supervised MLP (Minimal):} A two-layer feedforward neural network (multi-layer perceptron, MLP) with 169 parameters \ref{
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 14]}}
...
The primary experiments in Sections \ref{sec:siso-bpsk-performance-baseline-relay-comparison} to \ref{sec:parameter-normalization-complexity-trade-off} use BPSK modulation to isolate the relay processing comparison from modulation complexity. To test whether the BPSK findings generalize to higher-order constellations, section \ref{sec:higher-order-modulation-scalability-constellation-aware-training} extends the evaluation to QPSK and 16-QAM, while Section \ref{sec:input-normalization-and-csi-injection} further extends to 16-PSK. This section defines the four modulation schemes and the I/Q splitting technique that enables real-valued AI relays to process complex-valued signals.
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 4, Eq. 4-3-30]}}
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 4, Eq. 4-3-31]}}
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 5, Eq. 5-2-60]}}
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 5, Eq. 5-2-79]}}
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 5, Eq. 5-2-1]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad psk16-mapping}
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 5]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad psk16-decision}
...
}\end{equation} \par{\small\textit{source: [\cite{LoykaGagnon2004VBLASTAnalysis}, Ch. 5, Eq. 5-2-22]}}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad psk16-ber}
