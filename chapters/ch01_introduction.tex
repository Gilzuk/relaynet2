...
\begin{quote}
\textbf{System model studied in this thesis.} To orient the reader before the supporting theory is introduced, the model evaluated throughout this work is fixed as follows: a \textbf{half-duplex, two-hop} link with a single relay and \textbf{no direct source--destination path}. The \textbf{first hop (S\(\rightarrow\)R) is a single-input single-output (SISO)} channel (AWGN, Rayleigh, or Rician), and the \textbf{second hop (R\(\rightarrow\)D) is a \(2\times 2\) MIMO channel with independent, identically distributed (i.i.d.) Rayleigh \emph{fast} fading} (a fresh channel realization per channel use), equalized at the destination (ZF, MMSE, or SIC). The primary modulation is BPSK, and the performance metric reported throughout is the \textbf{uncoded bit error rate (BER)} as a function of SNR. The remaining models discussed in this chapter --- the general information-theoretic relay channel (Section~\ref{sec:information-theoretic-foundations}), the MIMO capacity result, and the diversity--multiplexing tradeoff (Section~\ref{sec:mimo-systems-and-equalization}) --- are presented as \emph{theoretical context and bounds}, not as the operating regime that is simulated. The full system model is restated in Section~\ref{sec:system-model-1}.
\end{quote}

\subsection{Information-Theoretic Foundations (Background)}\label{sec:information-theoretic-foundations}

\emph{The information-theoretic results presented in this subsection are classical \textbf{theoretical context} for the Gaussian (line-of-sight, LOS, delay-sensitive) relay channel. They motivate decode-and-forward (DF) as a baseline. The channel actually simulated in this thesis is the NLOS, fast-fading regime defined in the system-model statement above; in that regime the relay-channel capacity problem is not delay-sensitive, and the metric reported throughout this work is uncoded BER --- not ergodic capacity.}
...
\subsection{Two-Hop Relay Model (Half-Duplex; Listening and Transmission Phases)}\label{sec:two-hop-relay-model}

In the half-duplex two-hop model studied in this thesis, the relay cannot transmit and receive in the same time/frequency resource, and there is no direct source--destination link. \emph{Half-duplex} means that, at every channel use, the relay is either receiving from the source or transmitting to the destination, but not both. Without loss of generality, the relay operation can therefore be split into two phases that are separated in time: a \textbf{listening phase}, during which the relay observes \(y_R[\cdot]\), and a \textbf{transmission phase}, during which the relay emits \(x_R[\cdot]\). Equivalently, the communication proceeds in two time slots, indexed by \(i\):

\textbf{Slot 1 --- Listening phase (Source $\rightarrow$ Relay):} \begin{equation}\protect\phantomsection\label{eq:awgn-hop1}{y_R[i] = x[i] + n_1[i], \quad n_1[i] \sim \mathcal{N}(0, \sigma^2)
}\end{equation} \par{\small\textit{Source: \cite{laneman2004cooperative}}}

\textbf{Slot 2 --- Transmission phase (Relay $\rightarrow$ Destination):} \begin{equation}\protect\phantomsection\label{eq:awgn-hop2}{y_D[i] = x_R[i] + n_2[i], \quad n_2[i] \sim \mathcal{N}(0, \sigma^2)
}\end{equation} \par{\small\textit{Source: \cite{laneman2004cooperative}}}

where \(x[i] \in \{-1, +1\}\) is the BPSK symbol transmitted at time index \(i\), \(y_R[i]\) is the signal received at the relay, and \(y_D[i]\) is the signal received at the destination. The noise terms are i.i.d.\ across time with variance \(\sigma^2 = P_s / \text{SNR}\). The relay output \(x_R[i] = f\!\left(y_R[i-w:i+w]\right)\) is computed during the transmission phase from the time-indexed observations collected during the listening phase, where \(w\) is the half-window size of the relay processing function (\(w=0\) for memoryless AF/DF, \(w>0\) for the AI relays studied in Chapters~\ref{sec:methods}--\ref{sec:experiments}). The half-duplex constraint introduces a spectral-efficiency penalty of a factor of two (two channel uses are needed per delivered symbol), but it is a standard assumption in practical relay standards and simplifies the analysis while remaining representative of practical relay-processing scenarios.
...
The fundamental trade-off in MIMO systems is between spatial multiplexing gain (transmitting multiple independent streams) and diversity gain (transmitting redundant copies to combat fading). For \emph{slow} (block-) fading channels with outage as the relevant metric, Zheng and Tse \cite{zheng2003diversity} formalized this as the diversity--multiplexing tradeoff (DMT): for an \(N_t \times N_r\) system at multiplexing gain \(r\), the maximum achievable diversity order is \(d(r) = (N_t - r)(N_r - r)\), which at full spatial multiplexing in the \(2 \times 2\) case yields \(d = 0\) (no diversity protection; the system is interference-limited). \emph{The DMT is a slow-fading / outage concept and is mentioned here only as background that motivates why the choice of equalization method is critical.} The Hop-2 channel evaluated in this thesis is, by contrast, \(2 \times 2\) i.i.d.\ Rayleigh \emph{fast} fading (a fresh realization per channel use), and the metric reported throughout is the \textbf{uncoded BER as a function of SNR} averaged over the fading ensemble --- not outage and not the DMT.
