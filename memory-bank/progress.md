...
- **`e6_viterbi_qpsk_pilot_overhead.py`** — realistic Viterbi-Est with 1%-pilot-overhead LS channel estimation (250 pilots / 25,000-symbol data block) vs Viterbi-Genie, QPSK, L=3 taps, symmetric hops, full scale (5×50k). Exercises `ViterbiMLSEQPSKRelay`'s pre-existing `pilot_symbols=` constructor path for the first time on QPSK. **Finding**: Viterbi-Est-1pct is consistently, slightly *better* than Viterbi-Genie at all 11 SNR points (~0.002–0.003 BER lower, e.g. 0.2275 vs 0.2292 @20dB) — confirmed real (same-sign at every SNR point), reported as an **unconfirmed hypothesis** (LS estimate may implicitly recalibrate to the pilot window's realized Rayleigh fading level) rather than an established mechanism. Practical takeaway: 1% airtime overhead is essentially free here — matches or beats genie CSI for a 3-tap channel.
