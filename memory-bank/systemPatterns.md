...
- **Hard vs soft decision relays** (added for the enhanced multi-architecture comparison): a "hard" DF relay quantizes to the nearest constellation point (`DecodeAndForwardRelay`); a "soft" DF relay does power normalization only, no quantization (`DFSoftRelay` in `e6_sim_enhanced.py` — not yet promoted into `relaynet/relays/`). For QPSK/QAM16, this hard/soft distinction is about symbol-level decisions, not just `sign()` — mirrors the multi-level modulation decision boundary problem.
