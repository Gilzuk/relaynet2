...
3. **No `calculate_ber()` for QPSK/QAM16**: `relaynet.modulation` only has `bpsk.calculate_ber`. `relaynet.modulation.qpsk` / `.qam` exist but lack this function. When the user asked for QPSK/QAM16 hard/soft DF comparisons, this blocked a literal port — resolved by implementing hard/soft decision logic directly inside relay classes (`e6_sim_enhanced.py`) and scoping the actual BER sweep to BPSK for now. **If QPSK/QAM16 BER curves are needed for real, `calculate_ber` needs to be added to those modulation modules first.**
