...
\textbf{Goal:} Test whether the learned relay's value changes qualitatively when the channel violates the assumptions under which AF and DF are (near-)optimal: do the \emph{memoryless} classical relays fail on an unknown channel while the minimal $170$-parameter MLP mitigates by learning it from data (H5)?\REV{Corrected: this relay is $11 \to 13 \to 1$, which is $170$ parameters, not the $169$ an earlier revision gave it. The canonical-setup relay of Chapter~\ref{sec:experiments} is $5 \to 24 \to 1$, genuinely $169$; the composite, blind and pilot-budget relays below take complex I/Q pairs, $22 \to 7 \to 1$, also genuinely $169$. Three networks of the same class and budget, two of which land on the same count by coincidence. The earlier text asserted all were $169$ and called the difference immaterial: the difference \emph{is} immaterial to every conclusion, each of which depends only on the relay being minimal, but the counts themselves are reported here rather than rounded to a common figure.}
...
An earlier version of this section reported a further result that did not survive: that the ordering between the learned relay and the classical benchmark had reversed. It was withdrawn once the benchmark was found to be model-mismatched. The conjecture, the two measurements run to test it and the reason both refuted it are recorded in Appendix~\ref{sec:withdrawn-qpsk-benchmark}; the mechanism that actually explains the earlier figures is the subject of the next paragraph.
