...
\textbf{Variational Autoencoders (VAEs)} \cite{KingmaWelling2014VAE} posit that data \(\mathbf{x}\) is generated from a latent variable \(\mathbf{z} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})\) through a decoder \(p_\theta(\mathbf{x} | \mathbf{z})\). The true posterior \(p(\mathbf{z} | \mathbf{x})\) is intractable, so an encoder \(q_\phi(\mathbf{z} | \mathbf{x})\) approximates it, and training maximizes the evidence lower bound (ELBO), which decomposes into a reconstruction term and a regularization term:
...
The reconstruction term encourages the decoder to reproduce the input from the latent code, while the KL term pulls the latent space toward the standard Gaussian prior. The \(\beta\) parameter (\(\beta\)-VAE) \cite{Higgins2017betaVAE} trades the two off: \(\beta < 1\) prioritizes reconstruction, which suits the relay task where accurate signal recovery is what matters, while \(\beta > 1\) encourages disentangled representations. Gradient-based training passes through the stochastic sampling layer by the standard reparameterization trick, \(\mathbf{z} = \boldsymbol{\mu} + \boldsymbol{\sigma} \odot \boldsymbol{\epsilon}\) with \(\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})\), which moves the stochasticity outside the computational graph.

For relay signal processing, the VAE learns a compressed latent representation of the clean signal manifold: at inference the noisy received signal is encoded, and the decoder maps it back to a denoised estimate, with the regularized latent space providing implicit denoising as inputs far from the learned manifold are pulled toward high-probability regions. Sampling \(\mathbf{z}\) stochastically at inference does, however, inject variance into an otherwise deterministic estimate; Section~\ref{sec:generative-vs.-discriminative-paradigms-for-relay-processing} reports what this costs in practice.
...
\textbf{Transformers} \cite{Vaswani2017Attention} use multi-head self-attention to capture global dependencies in sequences. The core mechanism computes a weighted combination of value vectors, where the weights are derived from the compatibility of query and key vectors:
...
where \(\mathbf{Q}, \mathbf{K} \in \mathbb{R}^{n \times d_k}\) and \(\mathbf{V} \in \mathbb{R}^{n \times d_v}\) are obtained from the input via learned linear projections, and the scaling factor \(1/\sqrt{d_k}\) keeps the dot products out of the softmax's saturation regions. Multi-head attention runs \(h\) such heads in parallel with independent projections and concatenates their outputs, so that different heads may attend to different aspects of the input: for signal processing, one head might focus on immediate neighbors (local denoising) while another captures longer-range patterns. Because attention is permutation-equivariant, position information must be injected explicitly; this thesis uses the standard sinusoidal positional encoding \cite{Vaswani2017Attention}.

The property that matters for the comparison made here is cost. The attention matrix \(\text{softmax}(\mathbf{Q}\mathbf{K}^T/\sqrt{d_k}) \in \mathbb{R}^{n \times n}\) computes pairwise interactions between all \(n\) positions, giving \(O(n^2)\) time and memory. For the 11-symbol relay window used in this thesis this is negligible (\(11^2 = 121\) entries), but the quadratic cost becomes prohibitive for longer sequences, which is what motivates the linear-time alternatives below.
