...
\REV{Fix applied: Eq.~\ref{eq:af-snr} now carries a citation, and the text states that it assumes CSI-assisted \emph{variable-gain} AF (the variant used here) and explains the origin of the convention-dependent $+1$ term.}
...
\REV{Fix applied (overclaim): ``can represent this function exactly'' held only for a \emph{fixed} $\sigma^2$. The claim is now split --- exact representation at fixed $\sigma^2$, close \emph{approximation} of the multi-SNR family --- and the latter is identified as an empirical finding rather than a consequence of the exact result.}
...
\REV{Fix applied (misdirected pointers): this sentence and the $\beta$-VAE sentence below previously pointed at the wrong equations; each parenthetical now references its intended target.}
...
\REV{Fix applied (VAE caveat): the paradigm-level conclusion is now qualified --- the VAE relay decodes stochastically at inference and the standard deterministic variant (posterior mean at test time) was never evaluated, so its gap may be an inference-time artifact rather than evidence about generative modelling.}
...
\REV{Fix applied: (1)~technical --- the step $\exp(\Delta_k\mathbf{A})\to\mathbf{0}$ requires $\mathbf{A}$ Hurwitz, so the parameterization $\mathbf{A}=-\exp(A_{\log})$ ($\mathbf{A}\prec0$) is now stated explicitly before it is used; (2)~editorial --- the bare \texttt{$\backslash$ref}, the missing space, the \texttt{addcontentsline} placed \emph{inside} an equation environment, and a duplicated entry were all corrected.}
...
\end{remark}
\REV{Fix applied (cross-referencing): this note was a \texttt{quote} environment whose label resolved to the enclosing \emph{section} number. It is now a numbered remark, and both citing sites reference it as Remark~\ref{noise-convension}.}
...
\REV{Fix applied: the stray literal ``R'' after \texttt{addcontentsline} was removed, and Eq.~\ref{eq:df-ber-hops} in Section~\ref{sec:classical-relay-strategies-df} now uses the same exact odd-flip form as Eq.~\ref{eq:df-ber-awgn}, with a sentence noting that two errors cancel --- so the two appearances no longer read as a discrepancy.}
...
\emph{This section is background only: no MIMO experiment is evaluated in this thesis --- the scope table of Chapter~\ref{sec:introduction} defers MIMO to future work. The material is retained solely to define the terms used in the future-work discussion (Chapter~\ref{sec:discussion-and-conclusions}); the corresponding equalizer implementations ship with the simulation framework (Appendix~C).} Three standard equalization methods for a 2×2 MIMO topology are:
...
\textbf{LMMSE:} \begin{equation}\protect\phantomsection\label{eq:mmse-equalizer-methods}{\hat{\mathbf{x}}_{\text{LMMSE}} = (\mathbf{H}^H\mathbf{H} + \sigma^2\mathbf{I})^{-1}\mathbf{H}^H\mathbf{y}
}\end{equation}
\addcontentsline{equ}{listequations}{\protect\numberline{\theequation}\quad mmse-equalizer-methods}

LMMSE (linear MMSE) adds a noise-variance regularization term that prevents excessive noise amplification, trading residual interference for better noise performance.

\textbf{LMMSE-SIC (V-BLAST):}
...
\tightlist
\item
  \textbf{Ordering:} Compute LMMSE post-detection SINR for each stream; select the stream with highest SINR first.
\item
  \textbf{Detection:} Apply LMMSE equalization to the selected stream and make a hard decision.
\item
...
SIC outperforms plain LMMSE because the second stream sees no inter-stream interference after cancellation. The cost is potential error propagation from incorrect first-stream decisions. Note that the equalizers named here are \emph{linear} MMSE throughout, per the supervisor's comment (Appendix~E, Chapter~2 comment~7); the unqualified term ``MMSE'' is reserved in this thesis for the genuinely non-linear MMSE-optimal estimator $\mathbb{E}[x\mid y]$ of the relay denoising task.
\REV{Fix applied (scope): Section~\ref{sec:theoretical-foundations-mimo-equalization} is now framed explicitly as background --- no MIMO experiment is evaluated in this thesis --- the implementation note was moved to Appendix~C, and the MMSE terminology was corrected to LMMSE per the supervisor's comment (Appendix~E).}
