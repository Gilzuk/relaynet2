...
\REV{Stray cross-reference: ``(Equation~\ref{eq:mamba-selective})'' has nothing to do with this sentence about autoencoders --- delete it. The same class of misplaced parenthetical \texttt{(Equation...)} pointers appears repeatedly in this chapter (e.g., a pointer to \ref{eq:mse-loss} inside the $\beta$-VAE sentence, and Section~2.2.4's ``to minimize: (Equation~\ref{eq:optimal-denoiser})'' which should point to \ref{eq:mse-loss}); they look like artifacts of an automated conversion pass. Sweep the whole document for \texttt{(Equation~$\backslash$ref\{...\})} and verify each one points at the equation actually being discussed.}
...
\REV{Typo in the source right above this line: \texttt{$\backslash$addcontentsline\{equ\}\{listequations\}R\{...\}} --- the stray ``R'' before the brace is typeset into the PDF as a literal letter after the equation. Remove it. Also note Eq.~\ref{eq:df-ber-awgn} here and Eq.~\ref{eq:df-ber-hops} in Section~2.1.2 are two different expressions for the same quantity ($P_{e,1}+(1-P_{e,1})P_{e,2}$ vs.\ $P_{e,1}+P_{e,2}-2P_{e,1}P_{e,2}$); they differ because the second counts an error only when exactly one hop errs (double-flip corrects itself) --- say so explicitly, otherwise it reads as an inconsistency.}
...
\REV{``{[}21, Eq.\ 14-4-15{]}'' is a hardcoded numeric citation left over from a pre-BibTeX draft --- replace with \texttt{$\backslash$cite} (this is Proakis \& Salehi, \cite{ProakisSalehi2008DigitalComm}). Grep for remaining ``{[}digit{]}''-style citations; at least one more (``{[}21, Ch.\ 14{]}'') survives elsewhere. Mixed citation styles are the kind of thing that costs credibility cheaply.}
...
\REV{Scope check on Section~2.6: the scope table in Chapter~1 states MIMO is \emph{future work} and no MIMO experiment appears anywhere in this thesis, yet this section says three equalizers ``were implemented'' and gives implementation/speedup details. Either (a) frame the section explicitly as background (``for completeness; not evaluated in this work'') and drop the implementation claims, or (b) if the ZF/MMSE/SIC code genuinely ships with the framework, say that in Appendix~C (software architecture), not in the literature review. As it stands, an examiner will ask ``where are the MIMO results?''}
