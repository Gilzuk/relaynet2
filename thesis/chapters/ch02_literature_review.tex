...
Ye et al.~\cite{YeLiJuang2018OFDMDL} showed that a DNN can jointly perform channel estimation and signal detection in OFDM systems, implicitly learning the channel's statistical structure well enough to remove the need for explicit pilot-based estimation. Samuel et al.~\cite{SamuelDiskinWunder2019MIMODetect} proposed DetNet, unrolling MIMO detection's iterative optimization into a fixed stack of network layers to reach near-maximum-likelihood performance at \(O(N_t^2)\) complexity against exhaustive ML search's exponential cost -- encoding the problem's iterative structure directly into the architecture. Sun et al.~\cite{Sun2018LearnToOptimize} applied deep learning to the NP-hard resource-allocation problem in interference networks, learning near-optimal power control policies orders of magnitude faster than conventional iterative algorithms.

Dorner et al.~\cite{Dorner2018OverTheAirDL} treated the entire communication system -- modulator, channel, demodulator -- as a single autoencoder trained end-to-end on bit error rate, jointly optimizing every component rather than designing each separately; the paradigm is conceptually close to relay processing, since both learn an encoder-decoder pair across a noisy channel.
...
The implemented relay is a supervised denoising VAE rather than the unconditional model above: its encoder receives the noisy observation window, while reconstruction is evaluated against the clean target. The latent regularization supplies a generative inductive bias, but the denoising property follows from this noisy-input/clean-target training objective rather than from the standard VAE derivation alone. Sampling \(\mathbf{z}\) at inference also injects variance into an otherwise deterministic estimation task; Section~\ref{sec:generative-vs.-discriminative-paradigms-for-relay-processing} reports the measured outcome without attributing a causal effect to that sampling.\REV{ This sentence previously attributed ``the consistent VAE under-performance observed in this thesis'' to inference-time sampling. That reading does not survive re-measurement (Section~\ref{sec:generative-vs.-discriminative-paradigms-for-relay-processing}, Table~\ref{tbl:table2}), so the claim is no longer asserted here.}
