...
\REV{Equation~\ref{eq:af-snr} is a standard result and needs a citation (e.g., Laneman et al.\ \cite{Laneman2004CooperativeDiversity} or Hasna \& Alouini on end-to-end SNR of AF relays); also state its assumption --- fixed-gain vs.\ CSI-assisted variable-gain AF --- since the ``$+1$'' term differs between conventions.}
...
\REV{``Represent this function exactly'' overclaims. A single tanh unit represents $\tanh(y/\sigma^2)$ exactly only for one \emph{fixed} $\sigma^2$; the deployed network is trained across SNR $\in\{5,10,15\}$~dB and evaluated across $0$--$20$~dB, so it must approximate a \emph{family} of posteriors with one set of weights (and, on fading, a mixture over effective SNRs). That it does so well is the finding --- soften to ``can represent the fixed-$\sigma^2$ posterior exactly, and approximate the multi-SNR family closely.''}
...
\REV{Fix applied: removed the stray ``(Equation~\ref{eq:mamba-selective})'' pointer, which had nothing to do with this autoencoder sentence. Note for the author: the same class of misplaced parenthetical \texttt{(Equation...)} pointers recurs in this chapter (e.g.\ inside the $\beta$-VAE sentence and in Section~2.2.4's ``to minimize: (Equation\ldots)''); worth a sweep, but the remaining ones are left in place as they require judging the intended target.}
...
\REV{Two points here. (1)~Technical: $\exp(\Delta_k \mathbf{A}) \to \mathbf{0}$ for large $\Delta_k$ holds only if $\mathbf{A}$ is Hurwitz (eigenvalues with negative real part) --- which Mamba guarantees by parameterizing $\mathbf{A} = -\exp(A_{\log})$ --- but that sign convention is never stated in this section, so Eq.~\ref{eq:mamba-selective-ak} is unjustified as written. State $\mathbf{A} \prec 0$ explicitly. (2)~Editorial: ``makes \ref{eq:mamba-selective-ak2} approximately unity'' uses a bare \texttt{$\backslash$ref} (renders as a number with no ``Eq.''), ``hence,preserving'' is missing a space, and Eq.~\ref{eq:mamba-selective-bk} has its \texttt{addcontentsline} \emph{inside} the equation environment plus a duplicated entry --- clean up this block.}
...
\end{quote}
\REV{This noise-convention note is very good --- most theses silently mix the $Q(\sqrt{\gamma})$ and $Q(\sqrt{2\gamma})$ conventions and leave the reader to reconcile a 3-dB discrepancy; here it is stated once, correctly, and referenced later. One mechanical fix: it is referenced in Section 2.5.1.2 via a bare \texttt{$\backslash$ref} to a label on a \texttt{quote} environment, which resolves to the enclosing \emph{section} number; use the page-reference form (as done in Chapter~\ref{sec:experiments}) or promote this note to a numbered remark so it can be referenced cleanly.}
...
\REV{Fix applied: removed the stray ``R'' from \texttt{$\backslash$addcontentsline\{equ\}\{listequations\}R\{...\}}, which was typesetting a literal ``R'' after the equation. Note for the author: Eq.~\ref{eq:df-ber-awgn} here and Eq.~\ref{eq:df-ber-hops} in Section~2.1.2 are two forms of the same quantity that coincide; consider noting that the second counts an error only when exactly one hop errs, so it does not read as a discrepancy.}
...
\REV{Fix applied: replaced the hardcoded ``{[}21, Eq.\ 14-4-15{]}'' with \texttt{$\backslash$cite[Eq.~14-4-15]\{ProakisSalehi2008DigitalComm\}}. Note for the author: at least one more hardcoded numeric citation (``{[}21, Ch.\ 14{]}'') survives elsewhere in this chapter --- worth a grep for the ``{[}digit{]}'' pattern.}
...
\REV{Fix applied (scope): Section~2.6 is now framed explicitly as background --- no MIMO experiment is evaluated in this thesis --- with the implementation note moved to Appendix~C, and the MMSE terminology corrected to LMMSE per the supervisor's comment (Appendix~E).}
