...
A line of work closer to this thesis's benchmark places the learned component
\emph{inside} an optimal detector rather than beside it. Tsai et
al.~\cite{TsaiTeng2020BCJRNet} replace the channel-model-dependent branch
likelihoods of the BCJR recursion with a neural network, and Karanov et
al.~\cite{Karanov2024BurstyISI} extend that construction to ISI channels with
bursty impulsive noise, in both cases to avoid needing full channel state
information. What matters here is the condition under which the learned variant
wins. Chen et al.~\cite{Chen2024BCJRNetRobustness} report that the neural-aided
BCJR outperforms the conventional algorithm when channel knowledge is
imperfect, which is the same boundary this thesis draws in
Chapter~\ref{sec:unknown-channels}: the learned component earns its place under
model mismatch, not against a correctly matched receiver. That literature also
fixes the comparator. It benchmarks against BCJR, the detector that minimizes
bit error probability, rather than against sequence-optimal MLSE; the present
work benchmarks against MLSE only, a gap recorded in
