...
\REV{Activation inconsistency: here (and in Eq.~\ref{eq:mlp-relay}) the MLP hidden layer is ReLU, but every experiment configuration line in Chapters~\ref{sec:experiments}--\ref{sec:extensions} lists ``NN activation: tanh''. One of the two is wrong --- fix whichever does not match the code, and make Appendix~B the single authoritative statement of each architecture. (The 169-parameter count $(5{\times}24+24)+(24{\times}1+1)$ checks out either way.)}
...
\REV{Monte-Carlo budget consistency: this section fixes $10\times10{,}000$ bits per SNR point, and the abstract asserts it for \emph{all} results --- but E1 (Section~\ref{sec:channel-model-validation-simulative}) uses $20\times50{,}000$ and the entire unknown-channel chapter uses $5\times50{,}000$. None of these choices is wrong, but the blanket claim is. Add a one-row-per-experiment budget table here and make the abstract point to it. Relatedly: with only $M=5$ trials the two-sided Wilcoxon signed-rank test \emph{cannot} reach $p<0.05$ (minimum attainable $p = 2^{-4}=0.0625$), so significance claims must be confined to the $M=10$ experiments --- state this where the 5-trial results are reported.}
