...
\textbf{BER resolution, and what an observed zero licenses.} With \(N \cdot M = 100{,}000\) total bits per SNR point, the smallest non-zero BER the budget can express is \(1/N = 10^{-4}\) within one trial, or \(10^{-5}\) across the aggregate. A cell in which no error occurs is \emph{not} evidence that the BER is zero, and is never reported as zero anywhere in this thesis. Observing zero errors in \(n\) bits bounds the error probability above by the rule of three, \(\hat{p}_{\max} = 3/n\) at one-sided \(95\%\) confidence \cite{HanleyLippmanHand1983ZeroNumerators}, so the aggregate budget here licenses only \(p < 3\times10^{-5}\) --- three times the nominal resolution, not zero and not \(10^{-5}\). Zero-error cells are therefore reported as that upper bound with the bit count that produced it, the convention Chapter~\ref{sec:unknown-channels} follows at its rare-event operating points (Table~\ref{tbl:tableE6}, where \(10^{10}\) error-free bits give \(3/N = 3.0\times10^{-10}\) rather than an exact zero). The same reading applies to any cell printed with a \(<\) sign: it is a confidence bound set by the exposure, not a measurement.
