...
This thesis evaluates the primary (constellation, channel) configurations listed in Table~\ref{tbl:configurations}, plus the QPSK/unknown-ISI generalization of Section~\ref{sec:qpsk-unknown-channel} noted there. The governing principle is that \textbf{a real channel is paired with a real constellation and a complex channel with a complex constellation}: the signal space the constellation lives in and the signal space the channel acts on are the same space. Rayleigh with QPSK is the complex case, both living in the plane, where the fading coefficient \(h \sim \mathcal{CN}(0,1)\) rotates as well as scales and the constellation has a phase for it to act on.
...
\caption[The evaluated configurations]{The primary evaluated (constellation, channel) pairings, each with the same channel on both hops; a QPSK generalization of the unknown-ISI study, with a different channel per hop, is reported separately in Section~\ref{sec:qpsk-unknown-channel}.}\label{tbl:configurations}\tabularnewline
\toprule\noalign{}
