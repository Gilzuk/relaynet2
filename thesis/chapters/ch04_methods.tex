...
The system under study is a two-hop relay network with a single relay node. All methods in this chapter are described for the canonical setup of Chapter~\ref{sec:introduction}: SISO on both hops, i.i.d.\ Rayleigh fast fading, complex baseband, BPSK and QPSK, uncoded BER.
...
\caption{Two-hop relay system model: source transmits through two channels with a neural network relay in between.Each hop applies the channel operator followed by coherent compensation (no equalizer is used in the canonical setup)}
...
This thesis evaluates exactly three (constellation, channel) configurations, listed in Table~\ref{tbl:configurations}. They are not a cross product: each constellation is paired with the one channel on which it answers a distinct question, and the remaining combinations are deliberately not run.

\begin{longtable}[]{@{}lllp{0.42\linewidth}@{}}
\caption{The three evaluated configurations. The channel shown applies to both hops.}\label{tbl:configurations}\tabularnewline
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
BPSK & AWGN & Calibration & AWGN is the only channel here with exact closed forms, \(Q(\sqrt{2E_b/N_0})\) single-hop and \(2P(1-P)\) two-hop DF, so it is where the simulator can be \emph{proved} correct rather than merely compared. BPSK is the constellation those closed forms describe. \\
QPSK & Rayleigh & Canonical & Rayleigh fast fading is the channel this thesis draws its conclusions on. QPSK places one bit on each axis the complex baseband already carries, so the identical I/Q-split relays process it unchanged. \\
16-QAM & Rayleigh & Extension & Four levels per axis break the per-axis relay formulation, which is precisely what Chapter~\ref{sec:extensions} studies; it is examined on the canonical channel, not on AWGN. \\
\end{longtable}

The three omitted pairings are omissions by design. QPSK and 16-QAM on AWGN are not reported because AWGN serves here as a measuring stick, and a measuring stick needs only the one constellation whose closed form is being checked; evaluating relays on AWGN at higher orders would invite conclusions from a channel the thesis deliberately draws none from (Appendix~\ref{sec:appendix-response-to-comments}). BPSK on Rayleigh is not reported because QPSK already occupies the canonical slot and is processed by the same relays, so the two would answer the same question twice.
