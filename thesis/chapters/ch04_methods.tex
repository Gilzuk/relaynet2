...
\REV{Fix applied (system model): Eq.~\ref{eq:relay-received-siso} previously showed a \emph{fixed} noise variance, i.e.\ the AWGN calibration case, although the canonical channel is Rayleigh with a random per-symbol effective SNR. It is rewritten in post-compensation form with $|h_1|^2\gamma$ stated. Separately, the channel \emph{operator} in Eq.~\ref{eq:hop-model-selection} was renamed $\mathcal{H}(\cdot)$ to stop colliding with the fading \emph{coefficient} $h$.}
