...
\REV{Fix applied: Eq.~\ref{eq:relay-received-siso} originally showed a \emph{fixed} noise variance, i.e.\ the AWGN calibration case, although the canonical channel is Rayleigh with a random per-symbol effective SNR. A first correction rewrote it in phase-compensation form, $|h_1|x + \tilde{n}$ with fixed $\sigma^2$, which stated the effective SNR correctly but still did not describe the statistic the simulator forms: \texttt{relaynet/channels/fading.py} divides by $h$. It is now in zero-forcing form, consistent with Eq.~\ref{eq:awgn-hop1} and with the implementation. Separately, the channel \emph{operator} in Eq.~\ref{eq:hop-model-selection} was renamed $\mathcal{H}(\cdot)$ to stop colliding with the fading \emph{coefficient} $h$.}
