...
This thesis compares classical and learned relay strategies for two-hop communication. One configuration is fixed throughout the core study: a single-relay SISO link with i.i.d.\ Rayleigh fast fading on both hops, complex baseband, Gray-coded QPSK and uncoded transmission, leaving the relay function as the only variable. Eight strategies are evaluated, from amplify-and-forward (AF) and symbol-wise decode-and-forward (DF) to generative and sequence models. Results are Monte Carlo estimates with 95\% confidence intervals, calibrated against the closed-form composition.

Under this benchmark the best learned relays outperform AF at low SNR and match DF at medium-to-high SNR; DF achieves the lowest BER at no parameter cost, an empirical, uncoded per-symbol claim, not general optimality. At high SNR, capacity rather than architecture governs performance: a $169$-parameter multilayer perceptron (MLP) attains the BER of models two orders of magnitude larger. A coded study raises that baseline: a rate-1/2 convolutional code with a \emph{block}-DF relay, unlike symbol-wise DF, beats uncoded DF from $8$~dB upward. The learned relay overtakes it at $20$~dB: a decoding relay spends the code's redundancy, a denoising one leaves it intact. Adapting the modulation-and-coding scheme makes rate selection dominate the relay-decoding question by an order of magnitude---unless a latency deadline reopens the gap.



The principal contribution is a ladder of four layers, each removing exactly one assumption from the layer above. At layer~1 classical processing wins: on a memoryless matched channel hard slicing is the optimal per-symbol relay function. Layer~2 adds an unknown three-tap intersymbol-interference filter: the memoryless classical relays fail, error rate rising with transmit power; a $169$-parameter windowed MLP restores the link at constant per-symbol cost, $1$--$1.5$~dB behind a Viterbi detector whose cost is exponential in channel memory. Layer~3 degrades the channel estimate: the classical advantage holds to roughly twenty pilots and is lost by ten. Layer~4 redraws the channel every block, where the learned relay leads the best blind classical equaliser.

The results delineate operating regimes: classical processing where the model matches the channel, and the learned relay where the channel is unknown, poorly estimated, or has no per-block estimate.
