...
This thesis compares classical and learned relay strategies for two-hop communication. One configuration is fixed throughout the core study: a single-relay SISO link, i.i.d.\ Rayleigh fast fading on both hops, complex baseband, Gray-coded QPSK, uncoded transmission, and the relay function as the only variable. Eight strategies are evaluated, from amplify-and-forward (AF) and symbol-wise decode-and-forward (DF) to generative and sequence models. Results are Monte Carlo estimates with 95\% confidence intervals, calibrated against the closed-form two-hop composition.

Under this benchmark the best learned relays outperform AF at low signal-to-noise ratio (SNR) and match DF at medium and high SNR, while DF achieves the lowest BER at no parameter cost---a claim about uncoded, per-symbol relaying, not general optimality. At high SNR capacity, not architecture, governs performance: a $169$-parameter multilayer perceptron attains the BER of models two orders of magnitude larger. A supplementary coded study raises that baseline: an outer convolutional code with a \emph{block}-DF relay, unlike the symbol-wise DF above, beats uncoded DF from $8$~dB upward. The learned relay overtakes it at $20$~dB, because a decoding relay spends the code's redundancy while a denoising one leaves it for the destination. Let the modulation and coding scheme adapt, and its selection dominates the relay-decoding question by an order of magnitude.



The principal contribution is a ladder of four layers, each removing exactly one assumption from the layer above. At layer~1 classical processing wins: on a memoryless matched channel hard slicing is the optimal per-symbol relay function. Layer~2 adds an unknown three-tap intersymbol-interference filter: the memoryless classical relays fail, their error rate rising with transmit power, while a $169$-parameter windowed relay restores the link at constant cost per symbol, $1$--$1.5$~dB behind a Viterbi detector whose trellis cost is exponential in channel memory. Layer~3 degrades the channel estimate: the classical advantage holds to roughly twenty pilots and is lost by ten. Layer~4 redraws the channel every block, where the learned relay leads the best blind classical equaliser.

The results delineate operating regimes rather than declaring a winner: classical processing where the assumed model matches the channel, the learned relay where the channel is unknown, poorly estimated, or has no per-block estimate.
