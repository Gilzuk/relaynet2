...
Under this benchmark the best learned relays outperform AF at low signal-to-noise ratio (SNR) and match DF at medium and high SNR, while DF achieves the lowest BER at no parameter cost---a claim about uncoded, per-symbol relaying, not general optimality. At high SNR, capacity rather than architecture governs performance: a $169$-parameter multilayer perceptron attains the BER of models two orders of magnitude larger. A supplementary coded study raises that baseline: a rate-1/2 convolutional code with a \emph{block}-DF relay, unlike the symbol-wise DF above, beats uncoded DF from $8$~dB upward. The learned relay overtakes it at $20$~dB, because a decoding relay spends the code's redundancy while a denoising one leaves it for the destination. Let the modulation and coding scheme adapt, and its selection dominates the relay-decoding question by an order of magnitude---unless a latency deadline is imposed, which reopens the gap.
...
The results delineate operating regimes rather than declaring a winner: classical processing where the assumed model matches the channel, and the learned relay where the channel is unknown, poorly estimated, or has no per-block estimate.
