...
This thesis compares classical and learned relay strategies for two-hop communication. One configuration is fixed throughout: a single-relay SISO link with i.i.d.\ Rayleigh fast fading on both hops, complex baseband, Gray-coded QPSK and uncoded transmission, leaving the relay function as the only variable. Eight strategies are evaluated, from amplify-and-forward (AF) and symbol-wise decode-and-forward (DF) to generative and sequence models. Results are Monte Carlo estimates with 95\% confidence intervals; DF's measured BER matches the closed-form two-hop composition to within one percent.

Under this benchmark the best learned relays outperform AF at low signal-to-noise ratio (SNR) and match DF at medium and high SNR, while DF achieves the lowest BER at no parameter cost---a claim about uncoded, per-symbol relaying, not general optimality. At high SNR performance is governed by capacity, not architecture: a $169$-parameter multilayer perceptron attains the BER of models two orders of magnitude larger. A supplementary coded study raises that baseline: an outer convolutional code with a genuine \emph{block}-DF relay---distinct from the symbol-wise DF above---beats uncoded DF from $8$~dB upward. Learned relays still win at high SNR, because a decoding relay spends the code's redundancy while a denoising one leaves it for the destination. Let the rate adapt, though, and rate selection dominates the relay-decoding question by an order of magnitude.



The principal contribution is a ladder of four layers, each removing one assumption. At layer~1 classical processing wins: on a memoryless matched channel hard slicing is the optimal per-symbol relay function. Layer~2 adds an unknown three-tap intersymbol-interference filter: the memoryless classical relays fail, their error rate rising with transmit power, while a $169$-parameter windowed learned relay restores the link at constant cost per symbol, $1$--$1.5$~dB behind a Viterbi detector whose trellis cost is exponential in channel memory. Layer~3 degrades the channel estimate: the classical advantage holds to roughly twenty pilots, lost by ten. Layer~4 redraws the channel every block, where the learned relay leads the best blind classical equaliser.

The results delineate operating regimes rather than declaring a winner: classical processing where the assumed model matches the channel, the learned relay where it is unknown, poorly estimated, or has no per-block estimate.
