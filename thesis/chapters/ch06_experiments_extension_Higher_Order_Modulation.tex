...
Table~\ref{tbl:table14} covers the AWGN channel, on which QPSK reproduces the
BPSK curves closely. Because the canonical setup of this thesis is Rayleigh
rather than AWGN, the same comparison is repeated on the canonical channel in
Table~\ref{tbl:table14ray}, so that the modulation extension is anchored to the
channel every other conclusion is drawn on.
