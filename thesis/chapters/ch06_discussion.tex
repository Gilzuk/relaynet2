...
Among the AI methods, the low-SNR advantage over AF is realized by the \emph{minimal} feedforward relays, not by the larger sequence models. On the canonical Rayleigh channel (Table~\ref{tbl:table2}, 0~dB) the MLP, CGAN, and Hybrid relays achieve the lowest AI BER ($\approx 0.247$, essentially matching DF's $0.245$), while the higher-capacity Transformer and Mamba S6 relays are worse ($\approx 0.317$--$0.325$) and only the VAE is worse still. The advantage over AF is therefore \emph{not} driven by parameter count --- the smallest network already captures it --- which is fully consistent with the inverted-U complexity finding (H3) and the equal-capacity convergence of the normalized comparison (H4, Section~\ref{sec:normalized-parameter-comparison}).
\REV{Fix applied: this paragraph previously claimed ``Mamba S6 achieves the lowest BER \ldots ordering correlates with capacity,'' which is contradicted by Table~\ref{tbl:table2} (on the canonical channel Mamba/Transformer are among the \emph{worst} AI relays at low SNR, not the best; the old ordering held only marginally on the AWGN calibration data). The corrected statement matches the canonical-channel table and, in fact, reinforces the thesis's ``less is more'' (H3/H4) narrative. Minor maintainer note: this subsection's label still reads \texttt{...h1-partially-confirmed} although the heading is ``H1: Confirmed'' --- harmless to readers but worth renaming.}
...
\textbf{Training time paradox.} Despite its superior asymptotic complexity, Mamba S6 required roughly 4.5× longer to train than the Transformer (2{,}141~s vs.~474~s on CUDA; Table~\ref{tbl:table13}). This counter-intuitive result is explained by three compounding factors:
\REV{Fix applied (numerical inconsistency): the prose now uses Table~\ref{tbl:table13}'s figures (2{,}141~s / 474~s, $\approx$4.5$\times$) as the single source of truth, where it previously quoted 2{,}366~s / 597~s from a different run. If run-to-run timing variance is material it should be stated once alongside the table; otherwise the two chapters now agree.}
...
\item
  \textbf{Direction reversal.} At \(n = 11\): Mamba S6 (1.88 s) \textless{} Mamba-2 (4.11 s). At \(n = 255\): Mamba-2 (1.05 s) \textless{} Mamba S6 (2.69 s). The crossover occurs because the overhead of building the \(L \times L\) SSM matrix is amortised over longer chunks, while S6's per-step kernel launch cost grows linearly.
\end{enumerate}
...
\item
  ...
  \REV{Fix applied (units drift): ``$\sim$1~dB'' changed to ``$\sim$1\% BER'' to match the same claim in Chapter~\ref{sec:experiments} (Section~\ref{sec:parameter-normalization-complexity-trade-off}) and the H4 row of the outcome table, which both report a relative-BER gap.}
\item
...
\REV{Fix applied (review finding \#2, cont.): three orphaned conclusions were removed here --- (8)~``Mamba S6 is the strongest architecture across the higher-order modulation experiments'' (QAM16/PSK16 ``top-3'' rankings), (9)~``No neural relay outperforms DF across the full 48-variant combinatorial search'' (with 16-PSK figures), and (10)~``Input LayerNorm is universally beneficial \ldots'' (citing a nonexistent ``Section 4.13''). None of the supporting experiments exist in this restructured version (16-PSK, the CSI/LayerNorm ablation, and the 48-variant search were all cut --- see Appendix~E). The in-scope part of the DF-optimality claim is already covered by Conclusion~2 and the modulation-extension finding below.}
