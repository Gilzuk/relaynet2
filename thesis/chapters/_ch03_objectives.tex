...
\begin{enumerate}
    ...
    \item \textbf{Gap 3: State space models for physical-layer signal processing.} The Mamba architecture \cite{gu2023mamba} and its Mamba-2 SSD variant \cite{DaoGu2024TransformersSSM} have demonstrated strong performance in NLP and genomics, but their application to relay-level physical-layer signal processing remains unexplored. The theoretical connection between SSMs and classical adaptive filters makes this a natural research direction, but empirical validation is needed.
\REV{Fix applied: the Mamba architecture was previously attributed to the earlier S4 paper; it now cites \cite{gu2023mamba}, and the novelty claim is scoped to relay-level processing rather than all of physical-layer communication.}
\end{enumerate}
