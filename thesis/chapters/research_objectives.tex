...
\end{longtable}
\REV{Table fixes: (1) restored HTML artifacts \texttt{\&gt;}$\to$\texttt{>} and \texttt{\&amp;}$\to$\texttt{\&} for compilation; (2) ``inverted-U study''$\to$``complexity study (U-shaped BER vs.\ model size)'' to stay consistent with the U-shaped-in-BER wording adopted in Chapter~\ref{sec:literature-review}; (3) unified ``Mamba $S_6$''$\to$``Mamba-S6''; (4) en-dash for the ``1--2'' range.}
...
\item \textbf{H3 (U-shaped complexity curve).} There exists an optimal model size for relay denoising beyond which performance degrades due to overfitting; equivalently, BER is U-shaped in model size (inverted-U in accuracy). Specifically, models with $\sim$100--200 parameters achieve performance comparable to models with $10$--$100\times$ more parameters.\REV{ Retitled ``Inverted-U''$\to$``U-shaped'' and made the axis explicit (U-shaped in BER $=$ inverted-U in accuracy), removing the inconsistency with the U-shaped wording now used in Chapter~\ref{sec:literature-review}. En-dashes applied to the parameter ranges.}
