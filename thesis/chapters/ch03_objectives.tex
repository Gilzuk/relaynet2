...
\item
  \textbf{Delineate the conditions under which a learned relay surpasses classical processing} on channels whose structure the classical relays do not model --- unknown memory, nonlinear asymmetry, and composite cascades of both --- by locating the boundary rather than demonstrating a capability: a memoryless control where learning is expected to buy nothing, a matched-receiver reference (Viterbi MLSE with genie CSI, which is sequence-ML rather than a bound on bit error rate --- the BER-optimal comparator is the bit-wise MAP rule, benchmarked separately in Section~\ref{sec:bcjr-benchmark}), a pilot-budget sweep to find the crossover, and a blind regime in which no channel estimate is available to the receiver at all (Chapter~\ref{sec:unknown-channels}).
\item
