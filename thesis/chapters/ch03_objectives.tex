...
To systematically evaluate and compare classical and AI-based relay strategies for two-hop communication on a single canonical setup (SISO on both hops, i.i.d.\ Rayleigh fast fading, complex baseband, BPSK, uncoded BER) and to determine the most effective relay strategy as a function of SNR regime and computational constraints.
...
\item \textbf{H3 (U-shaped complexity curve).} There exists an optimal model size for relay denoising beyond which performance degrades due to overfitting; equivalently, BER is U-shaped in model size (U-shaped in accuracy). Specifically, models with $\sim$100--200 parameters achieve performance comparable to models with $10$--$100\times$ more parameters.\REV{ Retitled ``U-shaped''$\to$``U-shaped'' and made the axis explicit (U-shaped in BER $=$ U-shaped in accuracy), removing the inconsistency with the U-shaped wording now used in Chapter~\ref{sec:literature-review}. En-dashes applied to the parameter ranges.}
