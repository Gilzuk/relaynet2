...
Unlike H1--H4, which assume the canonical memoryless channel throughout, H6 deliberately introduces impairments not present in the system model of Chapter~\ref{sec:introduction}, including channel memory, ISI, and nonlinear distortion.
\begin{enumerate}
...
\tightlist
\item
  \textbf{Modulation:} The experiments use QPSK on the canonical channel throughout. Higher-order constellations (16-QAM, 16-PSK, 64-QAM and denser), together with the joint 2D $N$-class relay formulation they require, are deferred to future work.
\item
  \textbf{Channel knowledge:} \emph{In the canonical core} (Chapter~\ref{sec:experiments}), perfect CSI is assumed at the receiver and channel estimation errors are not modeled. This limitation is deliberately lifted in the principal contribution: Chapter~\ref{sec:unknown-channels} models finite-pilot least-squares channel estimation (a 200-pilot Viterbi baseline and a swept pilot budget) and, in the blind regime, a fully zero-CSI setting with no channel prior at all.
