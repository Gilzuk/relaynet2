...
\item \textbf{H3 (U-shaped complexity curve).} There exists an optimal model size for relay denoising beyond which performance degrades due to overfitting; equivalently, BER is U-shaped in model size (U-shaped in accuracy). Specifically, models with $\sim$100--200 parameters achieve performance comparable to models with $10$--$100\times$ more parameters. Only the second, weaker part of this hypothesis was testable with the protocol originally adopted: establishing \emph{overfitting} as the mechanism behind any upturn would require seed replication and train/test-gap measurement. The seed replication has since been performed for the MLP size axis by the sweep of Section~\ref{sec:minimum-relay-size}, which finds no upturn above the across-initialization spread (Section~\ref{sec:complexity-curve-turn}); the train/test-gap measurement, and any replication of the \emph{cross-architecture} comparison, remain outstanding and are recorded in Section~\ref{sec:limitations}.\REV{ Retitled ``U-shaped''$\to$``U-shaped'' and made the axis explicit (U-shaped in BER $=$ U-shaped in accuracy), removing the inconsistency with the U-shaped wording now used in Chapter~\ref{sec:literature-review}. En-dashes applied to the parameter ranges.}
