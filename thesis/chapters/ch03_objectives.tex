...
\REV{H6 as formulated here is the strongest part of the hypothesis set: it is falsifiable, pre-commits to the honest comparison (against estimate-then-MLSE rather than only against AF/DF), and explicitly predicts its own boundary. This is the right way to pose a learning-vs-classical claim.}
\end{enumerate}
...
\item
  ...
  \REV{Stale relative to the restructure: Chapter~\ref{sec:unknown-channels} explicitly models finite-pilot LS channel estimation (200-pilot and swept-budget Viterbi baselines) and even a zero-CSI blind regime. Qualify this item as ``in the canonical core (Chapters~\ref{sec:experiments}--\ref{sec:extensions})''; as written it contradicts the thesis's own principal contribution.}
\item
