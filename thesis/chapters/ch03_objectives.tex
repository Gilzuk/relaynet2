...
The two boundaries do not coincide, and the distance between them is a result in its own right rather than a restatement. O1's boundary is set by \emph{identifiability}: the learned relay overtakes MLSE when the channel can no longer be estimated well enough for MLSE to be run as specified, which Section~\ref{sec:partial-posterior} locates at a pilot budget between ten and twenty symbols. O2's boundary is set by \emph{arithmetic}, and Section~\ref{sec:joint-latency-memory} places it at a channel memory of roughly three and a half taps, exactly (Equation~\ref{eq:mac-crossover}) and without reference to any particular processor.

Latency, measured rather than assumed, turns out not to be a boundary at all on channels of short memory: an MLSE with bounded traceback reaches its full accuracy after three symbols of decision delay, fewer than a windowed relay needs, so a latency budget tight enough to exclude the classical detector excludes the learned one first. This is a negative result about a constraint the literature commonly treats as decisive, and Section~\ref{sec:joint-latency-memory} reports it as such.
...
\end{longtable}
\REV{Table fixes: ``U-shaped study''$\to$``complexity study (does BER rise again with model size?)'' to stay consistent with the U-shaped-in-BER wording adopted in Chapter~\ref{sec:literature-review}; (3) unified ``Mamba $S_6$''$\to$``Mamba-S6''; (4) en-dash for the ``1--2'' range.}
