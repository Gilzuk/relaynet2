...
\REV{This claim contradicts the canonical-channel data: in Table~\ref{tbl:table2} (Rayleigh, 0~dB) Mamba~S6 is at $0.317$ while MLP and CGAN are at $0.247$ --- Mamba is among the \emph{worst} AI relays at low SNR on the canonical channel, and the stated ordering holds (marginally, $0.2585$ vs.\ $0.2589$) only on the AWGN calibration table in the appendix. Specify which channel the ordering refers to, or better, re-write around the canonical channel where the ranking is MLP/CGAN/Hybrid first. As written, a reader who flips back to Table~\ref{tbl:table2} will catch this immediately. Also note the section label still reads \texttt{...h1-partially-confirmed} while the heading says ``H1: Confirmed'' --- stale label from an earlier draft; harmless to the reader but confusing to maintainers.}
...
The relay denoising task maps a window of \(2w+1\) real-valued noisy observations to a single real-valued clean estimate. For BPSK, the transmitted symbol carries exactly 1 bit of information, and under the canonical i.i.d.\ model the sufficient statistic for the current bit is the current observation alone, with Bayes-optimal posterior mean \(f^*(y_i) = \tanh(y_i / \sigma^2)\). The neighboring window samples are therefore redundant architectural inputs in this setting rather than part of the sufficient statistic, so the \textbf{effective dimensionality} of the target mapping is still 1: far below the \(2w+1 = 5\) or \(11\) input dimensions.
...
\REV{Numerical inconsistency: the timing table in Chapter~\ref{sec:experiments} (Table~\ref{tbl:table13}) reports Mamba~S6 at 2{,}141~s and Transformer at 474~s, while this paragraph says 2{,}366~s and 597~s. Presumably different runs --- pick one source of truth, cite the table, and if run-to-run variance matters, say so. Inconsistent timings across chapters invite the question of what else changed between runs.}
...
\item
  ...
  \REV{Units drift: here the converged gap is ``$\sim$1~dB'', in Chapter~\ref{sec:experiments} it is ``$\sim$1\% BER'', and H4's outcome table says ``within $\sim$1\% BER''. A 1-dB horizontal gap and a 1\% relative-BER vertical gap are very different claims at these operating points --- pick the metric actually measured and use it in all three places.}
\item
...
\REV{Conclusions 8--10 are orphaned: the ``48-variant combinatorial search'', the 16-PSK experiments, the QAM16/PSK16 ``top-3'' rankings, and the LayerNorm study are not described anywhere in this version of the thesis (16-PSK is explicitly out of scope per Chapter~1, and ``Section 4.13'' is a hardcoded number from a previous chapter structure that resolves to nothing here). A conclusion may only summarize evidence the document presents. Either (a) delete conclusions 8--10 and the corresponding hypothesis-table caveats, or (b) reinstate the supporting experiments in an appendix and cross-reference them properly. This is the most serious internal-consistency defect in the thesis --- an examiner reading conclusion~9's ``sweeps all three positions'' will search for the PSK16 table and find nothing.}
