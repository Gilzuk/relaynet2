...
\REV{Fix applied: ``can represent this function exactly'' held only for a \emph{fixed} $\sigma^2$. The claim is now split (exact representation at fixed $\sigma^2$, close \emph{approximation} of the multi-SNR family) and the latter is identified as an empirical finding rather than a consequence of the exact result.}
...
\REV{Fix applied: this sentence and the $\beta$-VAE sentence below previously pointed at the wrong equations; each parenthetical now references its intended target.}
...
\REV{Fix applied: the paradigm-level conclusion is now qualified, the VAE relay decodes stochastically at inference and the standard deterministic variant (posterior mean at test time) was never evaluated, so its gap may be an inference-time artifact rather than evidence about generative modelling.}
...
\REV{Fix applied: (1)~technical, the step $\exp(\Delta_k\mathbf{A})\to\mathbf{0}$ requires $\mathbf{A}$ Hurwitz, so the parameterization $\mathbf{A}=-\exp(A_{\log})$ ($\mathbf{A}\prec0$) is now stated explicitly before it is used}
...
\end{remark}
\REV{Fix applied: this note was a \texttt{quote} environment whose label resolved to the enclosing \emph{section} number. It is now a numbered remark, and both citing sites reference it as Remark~\ref{noise-convension}.}
