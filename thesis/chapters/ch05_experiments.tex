...
The compute column points the same way as the latency column, and the two machines make clear which parts of it can be relied on. Machine B is uniformly about $40\%$ slower than Machine A -- Viterbi costs $21.23\,\mu$s per symbol there against $15.07$ -- so no absolute figure in this table should be read as a property of the algorithms. What survives the change of machine is the structure. The learned relay is faster than Viterbi by $50\times$ on Machine A and $63\times$ on Machine B: the factor moves, the order of magnitude does not. BCJR costs $1.944\times$ Viterbi on Machine A and $1.941\times$ on Machine B, a ratio reproduced to three decimal places across hardware that differs by $40\%$ in absolute speed, exactly as its forward, backward and marginalization passes imply. The two MLP read-outs, by contrast, are \emph{not} separable: hard is cheaper on Machine B and soft is cheaper on Machine A, and repeat runs on one machine flip the ordering as well, so the correct statement is that replacing an \texttt{argmax} with a posterior-mean inner product costs nothing measurable -- not that either is faster. Taken together with the buffering column, which is exact and architectural rather than measured, the ordering is not in question even though the absolute timings clearly are.
