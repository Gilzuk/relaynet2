...
\REV{Scope contradiction: Table~\ref{tbl:table16} reports a ``Mamba S6 (+CSI + LN)'' variant, but the chapter opening (and Chapter~1's scope table) explicitly lists \emph{CSI injection} as future work not studied in this thesis, and no methods section defines what ``CSI injection'' or the LN placement means here. Either (a) drop this table and let the 2D-classification study carry the 16-QAM story (my recommendation --- it is the cleaner result), or (b) restore a short methods subsection defining the CSI+LN variant and adjust the scope statements. Related: Conclusions 8--10 in Chapter~\ref{sec:discussion-and-conclusions} still cite a ``48-variant combinatorial search,'' 16-PSK results, and a hardcoded ``Section 4.13'' --- none of which exist in this version of the document. This is the single most important consistency repair in the thesis.}
...
\end{figure}
\REV{Caption of Figure~\ref{fig:fig25} is truncated mid-sentence (``...at low SNR (Y'') --- restore the missing text. Also sweep captions for encoding damage: the caption of Figure~\ref{fig:fig50} contains a double-encoded ``$\approx$'' (rendering as mojibake ``â‰ˆ'').}
