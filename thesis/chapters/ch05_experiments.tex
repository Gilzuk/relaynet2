...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 18\tabcolsep) * \real{0.1000}}
  >{\raggedright\arraybackslash}p{(\linewidth - 18\tabcolsep) * \real{0.1000}}@{}}
\caption{BER comparison on the AWGN channel (SISO), companion to Table~\ref{tbl:table2}. Tabulated only to 12~dB: above that the true BER falls below what this Monte Carlo budget can resolve, so higher-SNR entries would report the resolution floor rather than a measurement (see the text).}\label{tbl:table2awgn}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
SNR (dB)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
