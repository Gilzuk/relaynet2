...
\REV{``inverted-U relationship between model complexity and relay performance'' \(\to\) ``overfitting at excess capacity'', consistent with H3 as retitled in Chapter~\ref{sec:research-objectives}.}
...
\REV{``the inverted-U result (H3)'' \(\to\) ``the minimal-capacity result (H3)''.}
...
Section~\ref{sec:joint-latency-memory} measures these components together on a channel with memory, and two of the arguments above do not survive the measurement. The first is the assumption, implicit in treating buffering as the learned relay's advantage, that sequence detection is latency-expensive. It is not: an MLSE with bounded traceback reaches its full accuracy after three symbols of decision delay on a three-tap channel, because the trellis survivors merge after three symbols and waiting longer revises no decision. That is \emph{fewer} symbols than the window-11 relay's five, so a latency budget tight enough to exclude MLSE excludes the learned relay first. Latency separates the block relays from everything else, and nothing else from anything.

The second is the appeal to wall-clock measurement. That the learned relay's measured advantage over Viterbi spans a factor of $50$ to $63$ \emph{depending on the machine} (Table~\ref{tbl:table41}) is itself the objection: the comparison sets a vectorized matrix multiplication against an interpreted trellis loop, so the ratio is substantially an artifact of the reference implementations, as Section~\ref{sec:complexity-viterbi-mlp} already records. Counting multiply-accumulates instead removes the processor from the argument and sharpens it into a threshold: MLSE costs $2M^{L}$ MACs per symbol against the relay's $2WH+4H$, which does not depend on channel memory at all, so the two are equal at $L^{\star}=3.35$ taps (Equation~\ref{eq:mac-crossover}). Below that the learned relay is not merely no cheaper but strictly worse on every axis at once --- at $L=3$ MLSE needs $128$ MACs against the relay's $208$, decides in three symbols against five, and is the more accurate detector. Above it the gap compounds as $M^{L}$, reaching $158\times$ at seven taps. The learned relay's cost advantage is therefore real but conditional, and the condition is a property of the channel rather than of the processor it runs on.
...
H5 & Memoryless classical relays fail on unknown channels; a minimal MLP mitigates by learning, near the model-aware optimum & \textbf{Confirmed}: on unknown ISI, AF/DF hit the analytic 0.25 floor (DF non-monotonic in SNR) while the 169-parameter MLP reaches BER $<5\times10^{-5}$, within $\approx$1--1.5 dB of genie-CSI Viterbi MLSE; the same architecture mitigates a composite ISI$\times$PA$\times$phase cascade with no impairment model, tracking $\approx$2 dB behind pilot-aided Viterbi and matching blind CMA when no posterior exists; the advantage is bounded to structural model-class mismatch representable at minimal capacity \\
\end{longtable}
...
\REV{H2/H3 statements in the outcome table aligned with the rest of the thesis: ``DF optimal at high SNR'' \(\to\) ``DF lowest-BER among per-symbol relays at high SNR''; ``Inverted-U complexity curve'' \(\to\) ``Overfitting at excess capacity''. \texttt{\$\&lt;5\ldots\$} restored to \texttt{\$<5\ldots\$}; all \texttt{\&amp;} restored to \texttt{\&}.}
...
These findings demonstrate that neural network-based relay processing is a viable complement to classical approaches in the low-SNR regime, matching, though not surpassing, the per-symbol-optimal DF baseline everywhere else at negligible computational cost. The overarching insight is that \textbf{model complexity should be matched to task complexity}: for the canonical relay denoising task, minimal architectures suffice, and the choice between neural network paradigms, i.e. feedforward or sequential, matters less than proper model sizing and regularization. The practical recommendation is to deploy a Hybrid relay with a 169-parameter MLP sub-network, which combines the neural low-SNR advantage over AF with DF's zero-parameter reliability at high SNR; for constellations with more than two levels per axis, a per-axis I/Q-split denoiser is not the right formulation and a joint two-dimensional relay would be needed, though that is not evaluated here; and on links with unmodeled impairments (impairments outside the canonical memoryless model, including ISI, bias, and hardware nonlinearity), a minimal learned relay delivers near-MLSE reliability without channel identification, where the memoryless classical relays fail outright (H5).
