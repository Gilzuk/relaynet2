...
\item
  \textbf{No coding, in the core comparison.} The core comparison operates without error-correcting codes, so the DF baseline there is its symbol-level variant (Remark~\ref{rem:df-terminology}). Sections~\ref{sec:coded-block-df}--\ref{sec:coded-soft-decision} measure a coded, information-theoretic block-DF baseline and its soft-decision counterparts directly rather than leaving this as an assertion, but that study is confined to one code family, rate and frame length, and the relay and destination each decide once with no iterative exchange between them.
\end{enumerate}
