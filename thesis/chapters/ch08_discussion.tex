...
\item
  \textbf{AF} applies \(f_{\text{AF}}(y) = Gy\), which is linear and preserves the noisy signal structure but amplifies the noise by the same factor as the signal. For the \emph{analytical} CSI-assisted variable-gain relay, in which the gain is recomputed from the instantaneous first-hop state, this costs \(\text{SNR}_{\text{eff}} = \gamma^2 / (2\gamma + 1) \approx \gamma/2\) at high SNR: the familiar 3~dB penalty. That expression is background, not an account of the curve measured here. The AF relay implemented in this thesis normalizes by a single block-adaptive scalar estimated from the empirical received power (Section~\ref{sec:classical-relay-strategies-af}), so its simulated BER is not predicted by the formula above and the $3$~dB figure should not be read as explaining it. What carries over is only the qualitative point that a linear relay amplifies noise and signal alike.
\end{itemize}
...
\item
  \textbf{Generalization.} The 169-parameter MLP handles the full Rayleigh fading ensemble, every effective SNR the fades produce, with a single set of weights and no per-condition retraining. Whether a larger model would generalize less well across the ensemble is not established here: every learned relay was trained and tested on the same fading distribution, and no out-of-distribution comparison by model size was run.
\end{enumerate}
...
\textbf{Best on the canonical QPSK benchmark} & Hybrid & Lowest BER at 0~dB and tracks DF above the crossover; measured only on the canonical memoryless QPSK/Rayleigh setup, with an empirically selected threshold. Not evaluated at BPSK, on channels with memory, blind, or under hardware constraints, and not a recommendation beyond that setup \\
\end{longtable}
