...
H4 is untouched in substance but acquires a caveat and a partial defence. The caveat is that the equal-budget architecture comparison remains single-seed, so it is exposed to exactly the initialization variance Table~\ref{tbl:seed-spread} measures. The partial defence is that on the canonical channel, where that comparison was run, the measured spread is the smallest anywhere in the sweep at $0.008$~dB median, comfortably below the roughly $0.18$~dB that H4's $4.1\%$ high-SNR spread corresponds to. That is reassurance rather than proof: the spread was measured on MLPs, and there is no basis here for assuming the sequence models initialize as stably.
