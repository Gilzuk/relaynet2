...
Among the AI methods, the low-SNR advantage over AF is realized by the \emph{minimal} feedforward relays, not by the larger sequence models. On the canonical Rayleigh channel (Table~\ref{tbl:table2}, 0~dB) the MLP, cGAN, and Hybrid relays achieve the lowest AI BER ($\approx 0.247$, essentially matching DF's $0.245$), while the higher-capacity Transformer and Mamba-S6 relays are worse ($\approx 0.317$--$0.325$) and only the VAE is worse still. The advantage over AF is therefore \emph{not} driven by parameter count, the smallest network already captures it, which is fully consistent with the U-shaped complexity finding (H3) and the equal-capacity convergence of the normalized comparison (H4, Section~\ref{sec:normalized-parameter-comparison}).
\REV{Fix applied (contradiction with Table~\ref{tbl:table2}): this paragraph previously claimed ``Mamba S6 achieves the lowest BER \ldots ordering correlates with capacity'', which the canonical-channel table contradicts (Mamba/Transformer are among the \emph{worst} AI relays at low SNR there; the old ordering held only marginally on the AWGN calibration data). The corrected statement reinforces the ``less is more'' narrative (H3/H4). ``inverted-U'' \(\to\) ``U-shaped''; architecture names unified.}
...
One of the most significant findings is the U-shaped relationship between model complexity and relay BER (equivalently, an inverted-U in accuracy). The Minimal MLP architecture (169 parameters) matches the performance of models with over 100-140$\times$ more parameters (3K--24K), while the Maximum MLP (11,201 parameters) exhibited clear overfitting with degraded performance.
...
\item
  \textbf{24,001 parameters (Mamba-S6):} Despite having 140$\times$ more parameters than MLP, the BER improvement at low SNR is only 1-2\%. The marginal parameter utility is vanishingly small.
\end{itemize}
...
\textbf{Training time paradox.} Despite its superior asymptotic complexity, Mamba-S6 required roughly 4.5$\times$ longer to train than the Transformer (2{,}141~s vs.~474~s on CUDA; Table~\ref{tbl:table13}). This counter-intuitive result is explained by three compounding factors:
\REV{Fix applied (numerical inconsistency): the prose now uses Table~\ref{tbl:table13}'s figures (2{,}141~s / 474~s, $\approx$4.5$\times$) as the single source of truth, where it previously quoted 2{,}366~s / 597~s from a different run. The MLP training time quoted elsewhere was likewise aligned to the table's 4.9~s, and the S6 inference time to 1.88~s.}
...
Table \ref{tbl:table25} presents the results.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Mamba-S6 becomes the bottleneck.} At \(n = 255\), the S6 sequential scan requires 255 serial CUDA kernel launches per layer per forward pass, making it 142 s for just 20 epochs of training: \textbf{28$\times$ slower} than the Transformer and \textbf{10.7$\times$ slower} than Mamba-2 SSD.
\item
  \textbf{Mamba-2 SSD recovers parallelism.} By chunking the 255-step sequence into 8 chunks of 32 and processing each chunk with a single batched matrix multiply, Mamba-2 SSD eliminates the sequential bottleneck. At 13.35 s, it trains \textbf{10.7$\times$ faster} than S6 and approaches the Transformer's speed.
\item
  \textbf{Inference parity.} Mamba-2 SSD inference (1.05 s) is nearly identical to the Transformer (0.99 s) and \textbf{2.6$\times$ faster} than Mamba-S6 (2.69 s). This is a complete reversal from the \(n = 11\) case, where Mamba-2 SSD was 2$\times$ \emph{slower} than S6.
\item
  \textbf{Direction reversal.} At \(n = 11\): Mamba-S6 (1.88 s) \textless{} Mamba-2 SSD (4.11 s). At \(n = 255\): Mamba-2 SSD (1.05 s) \textless{} Mamba-S6 (2.69 s). The crossover occurs because the overhead of building the \(L \times L\) SSM matrix is amortised over longer chunks, while S6's per-step kernel launch cost grows linearly.
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Modulation scope.} The core experiments use QPSK, the canonical constellation; BPSK is confined to the AWGN calibration channel, and the extension covers 16-QAM (Chapter~\ref{sec:extensions}). Denser constellations (64-QAM, 256-QAM), where the $N$-class 2D formulation scales to 64 or 256 output classes and the U-shaped complexity curve (H3) may shift toward larger models, are not investigated.
\item
...
\item
  \textbf{No coding, in the core comparison.} The core comparison operates without error-correcting codes, so the DF baseline there is its symbol-level variant (Remark~\ref{rem:df-terminology}). Section~\ref{sec:coded-block-df} now measures a coded, information-theoretic block-DF baseline directly rather than leaving this only as an assertion, but the learned relays evaluated there still produce hard, re-encoded decisions; a relay that instead forwards soft information (log-likelihood ratios) for the outer decoder to combine, which could change the optimization objective and potentially the relative standing of the strategies, remains untried.
\end{enumerate}
...
\item
  \textbf{Coded block-DF baseline (measured; open items remain).} As emphasized in Remark~\ref{rem:df-terminology}, the DF baseline in the core comparison is the uncoded, symbol-level variant; Section~\ref{sec:coded-block-df} now measures the coded, information-theoretic block-DF baseline this item used to only propose, over a rate-1/2 convolutional code (constraint length $K \in \{3,5,7\}$) with soft-decision Viterbi decoding, on both canonical constellations. The high-SNR optimality of symbol-wise DF (H2) does not carry over unmodified: coded block-DF beats it above roughly 6--8~dB, but is worse below that threshold, and two coded-aware learned relays (a windowed MLP and the Mamba-S6 architecture used elsewhere in this thesis) track but do not clearly beat the classical decoder. What remains open: only a single rate (1/2) and frame length (200 information bits) were swept, an adaptive code-rate-per-SNR-point scheme was not attempted, and the learned relays produce hard re-encoded decisions rather than soft information (log-likelihood ratios) for a downstream decoder to combine -- any of which could change the picture further.
\item
  \textbf{Mamba-2 at longer context lengths.} Our benchmark (Section \ref{sec:context-length-benchmark-validating-the-crossover-hypothesis}) demonstrates that Mamba-2 SSD achieves a 10.7$\times$ training speedup over Mamba-S6 at \(n = 255\). Future work should explore whether longer relay windows (e.g., 128-512 symbols) combined with the SSD architecture can improve both BER performance and processing speed simultaneously.
\end{enumerate}
...
H3 & U-shaped complexity curve (BER) & \textbf{Confirmed}: 169 params matches 24K; 11K params overfits \\
H4 & Architecture convergence at equal scale & \textbf{Confirmed at high SNR, rejected at low}: at 3K params the six architectures span 4.1\% BER at 20~dB but 17.4\% at 0~dB, splitting into feedforward and sequence groups \\
H5 & SSD faster than S6 at long context & \textbf{Confirmed}: 10.7$\times$ training speedup at \(n = 255\) \\
...
It is important to note that sequence-detection methods such as Viterbi MLSE appear only in the unknown-channel study of Chapter~\ref{sec:unknown-channels}, where channel memory is introduced explicitly through a 3-tap FIR/ISI channel. The canonical system model of Chapters~\ref{sec:introduction}-- \ref{sec:extensions} remains memoryless and therefore requires only per-symbol processing.
