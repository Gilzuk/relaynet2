...
The framework uses object-oriented design with a common \texttt{Relay} base class, enabling polymorphic relay swapping. Monte Carlo simulation is implemented in \texttt{runner.py} with configurable trial count, bit count, and SNR range. All tensor operations use vectorized PyTorch for GPU acceleration.

\subsection{Testing} 108 automated tests (pytest) cover the channels, modulation, relay-strategy, simulation and statistics modules with a 100\% pass rate. The suite requires only NumPy and SciPy; PyTorch is optional and needed solely by the neural relays that use it.
...
\paragraph{Re-measurement of all results.} Every simulation result reported in this thesis was re-measured after three faults were found in the earlier data: a $3$~dB error in the noise convention of the AWGN channel, which propagated into the \emph{training} data of every learned relay and so moved results on channels the fault never touched; three tables measured on (constellation, channel) pairings the thesis had withdrawn, each traceable to an unexamined default argument; and result files predating the implementation of the relays they reported. The published numbers are those of the re-measurement, at $10$ trials $\times\ 10{,}000$ bits per SNR point.

Three written conclusions changed as a consequence and are stated in their revised form in the body: hypothesis H4 is now confirmed at high SNR and rejected at low, rather than confirmed outright; the VAE relay does not fail, and the earlier failure is attributed to a superseded implementation rather than to the noise convention, which a controlled A/B rules out; and joint $16$-class classification is reported as restoring parity with decode-and-forward rather than as achieving near-zero error. A cell-by-cell record of the changes, the runtimes, and the reasoning is kept with the source as \texttt{thesis/RERUN\_CHANGELOG.md}, and \texttt{verify\_thesis\_tables.py} checks every published cell against the file that produced it.
\begin{figure}[bt]
\centering
\includegraphics[width=\textwidth,scale=1.0]{results/relaynet_sw_architecture.png}
...
\bottomrule\noalign{}
\endlastfoot
AWGN calibration & BPSK & AWGN & tab:ber\_val. & \url{results/calibration.json} \\
...
16-QAM joint 2D classifier & 16-QAM & Rayleigh & tbl:24 & \url{results/all_relays_16class/*.json} \\
Coded block-DF, $K$-sweep & QPSK, 16-QAM & Rayleigh & tbl:34--36 & \url{results/coded_df_experiment.json} \\
...
Unknown ISI (layer 2) & BPSK & AWGN / Rayleigh (2nd hop) & tbl:E6 & \url{e6_unknown_channel_results/e6_sim_ported_results.npy} + \url{e6_viterbi_*.npy} \\
Flat-channel control & BPSK & AWGN / Rayleigh & tbl:E6flat & \url{e6_flat_ported_results.npy} \\
Unknown ISI, QPSK generalization & QPSK & AWGN / Rayleigh & tbl:E6qpsk & \url{e6_qpsk_unknown_channel_results.npy} \\
Composite impairment cascade & DBPSK & AWGN / Rayleigh & E6composite & \url{e6_composite_ported_results.npy} \\
Partial posterior (pilot sweep) & BPSK & AWGN / Rayleigh & E6partial & \url{e6_partial_ported_results.npy} \\
Blind channel (layer 4) & BPSK & AWGN / Rayleigh & E6blind & \url{e6_blind_ported_results.npy} \\
Four-layer summary & mixed (above) & mixed (above) & tbl:layers & combination of the above \\
\end{longtable}
}

The result files above are themselves generated by named, committed scripts (e.g.\ \texttt{coded\_rate\_adaptation.py} produces \texttt{coded\_rate\_adaptation.json}); the correspondence between script and output file is one-to-one and is not separately tabulated here since the filenames make it explicit.
