...
The framework uses object-oriented design with a common \texttt{Relay} base class, enabling polymorphic relay swapping. Monte Carlo simulation is implemented in \texttt{runner.py} with configurable trial count, bit count, and SNR range. All batched tensor operations use vectorized PyTorch for GPU acceleration. The framework contains only components exercised by the reported experiments: the $2\times2$ MIMO equalizers that earlier versions carried have been removed along with the MIMO thread itself, so no unused signal-processing code remains (MIMO is deferred to future work, Chapter~\ref{sec:discussion-and-conclusions}).
