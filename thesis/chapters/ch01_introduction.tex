...
Figure~\ref{fig:canonical-relay-benchmark} summarizes this canonical benchmark: the source, the channel model, and the destination are identical across the canonical comparison, while only the relay function $f_\theta(\cdot)$ is varied there; the coded and unknown-channel studies deliberately vary the coding, channel, or destination processing as well. "Identical'' here refers to the model, not to any particular channel realization: the fading is \emph{fast}, so the coefficients $h_1[i]$ and $h_2[i]$ are redrawn independently for every symbol. Nothing in this thesis is a static or quasi-static link. The full system model and two terminology remarks are given in Section~\ref{sec:system-model-1}.
