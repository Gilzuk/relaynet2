...
Channel
&
i.i.d.\ Rayleigh fast fading (canonical); AWGN as the simulation baseline the framework is calibrated against (Section~\ref{sec:channel-model-validation-simulative}), with a companion relay comparison alongside the canonical one; unknown, mismatched, and time-varying channels are studied in the dedicated contribution (Chapter~\ref{sec:unknown-channels}) \emph{(future work: Rician and other channel families)} \\
...
Modulation
&
BPSK and QPSK, both canonical (Chapter~\ref{sec:experiments}); 16-QAM as a scoped extension (Chapter~\ref{sec:extensions}); QPSK is additionally re-checked under the unknown-ISI channel of Chapter~\ref{sec:unknown-channels} \emph{(future work: 16-PSK, 64-QAM and beyond)} \\
