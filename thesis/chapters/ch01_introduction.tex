...
SISO on both hops, i.i.d.\ Rayleigh fading, complex baseband, uncoded
BER, on the two constellations that baseband natively carries, BPSK and
QPSK. The only axis varied is the \emph{relay
function}. A scoped extension to 16-QAM (Chapter~\ref{sec:extensions}) and a dedicated unknown-channel contribution (Chapter~\ref{sec:unknown-channels}) are included;
...
\caption{Scope of the thesis: the canonical setup (BPSK and QPSK on Rayleigh), the 16-QAM extension, and the unknown-channel contribution. No other topology, relay protocol, or performance metric is evaluated in the main line of this work; all such variations are deferred to future work.}
\label{tbl:scope}\\
