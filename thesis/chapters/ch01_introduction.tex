...
\REV{Fix applied (editorial): ``Vs.''~$\to$ ``vs.'', and the ``as Existing'' run-on was split into two sentences.}
...
\REV{Fix applied (system-model consistency): the canonical hop equations now show the Rayleigh fading coefficient (Eq.~\ref{eq:rayleigh-hop1})--(Eq.~\ref{eq:rayleigh-hop2}), the coherent-compensation step is stated, and the former AWGN-only pair is retained explicitly as the $|h|\equiv1$ calibration limit, so the displayed math no longer contradicts the ``i.i.d.\ Rayleigh on both hops'' setup. $\gamma$ is now defined at first use below.}
