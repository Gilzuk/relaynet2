...
The window $y_R[i-w:i+w]$ uses look-ahead samples and is therefore \emph{not causal}. It is admissible in the half-duplex protocol studied here only because the relay buffers the entire listening-phase block before the transmission phase begins, so that when $x_R[i]$ is computed all of $y_R[i-w:i+w]$ has already arrived; the cost is a fixed processing latency of $w$ symbols. This is a limitation of the learned relays, not a property of the channel, and it is recorded as such. A strictly causal past-only window $y_R[i-2w:i]$ would remove it and is what a streaming deployment would require; that variant is not evaluated in this thesis and no claim is made for it. On the canonical memoryless channel $w=0$ would in any case suffice, since $y_R[i]$ is already a sufficient statistic for $x[i]$; the window is carried at $w>0$ so that the same relay architecture serves the unknown-channel study of Chapter~\ref{sec:unknown-channels}, where it must span the three-tap channel memory. AF and symbol-wise DF use $w=0$ and are strictly causal.
\end{remark}
