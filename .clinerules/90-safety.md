...
## Git Safety
- Always work on a feature branch off `main`; open a pull request back into `main`. `main` is the sole source of truth for `chapters/**` and the only branch this project targets. The Overleaf mirror at `Gilzuk/relaynet2-thesis` is generated by `scripts/overleaf_sync.py --repo`; it is a publish target, never a source, and is never edited directly.
