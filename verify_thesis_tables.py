...
def matched_viterbi(name, snr):
    ...
    Returns a point estimate where errors were observed and the rule-of-three
    95% upper bound where none were.
    """
    if snr in (16, 20):
        r = _MATCHED_HI["results"][name][str(snr)]
        return r["ber"] if r["errors"] else r["rule_of_three_upper_95"]
    i = _MATCHED["snrs"].index(snr)
    e = _MATCHED["results"][name]["errors"][i]
    b = _MATCHED["results"][name]["bits"][i]
    return (e / b) if e else 3.0 / b
